...
use super::types::{Effect, ToolSandboxError};

pub const MCP_CALL_RECEIPT_SCHEMA_VERSION: u64 = 1;
pub const MCP_CALL_RECEIPT_RECORD: u64 = 4;
...
impl McpCallReceipt {
    pub fn from_response(
        ...
        receipt.receipt_hash = expected_mcp_call_receipt_hash(&receipt);
        receipt
    }
    ...
    pub fn is_contract_valid(&self) -> bool {
        self.request_hash != 0
            ...
            && self.receipt_hash == expected_mcp_call_receipt_hash(self)
    }
    ...
    pub fn is_valid_for(&self, request: &McpCallRequest) -> bool {
...
impl LiveMcpCallExecutor {
    ...
    pub fn execute_call(
        ...
        tool_name: &str,
        args_json: &str,
    ) -> Result<McpCallReceipt, ToolSandboxError> {
        let request = self.request_for(tool_name, args_json)?;
        ...
        Ok(McpCallReceipt::from_response(
            &request,
            &bytes,
            exit_status,
            timed_out,
...
pub fn append_mcp_call_receipt_ndjson(
...

    {
        let mut file = OpenOptions::new()
            ...
            .open(path)
            .map_err(|_| ToolSandboxError::SandboxIo)?;
        writeln!(file, "{}", encode_mcp_call_receipt_ndjson(receipt))
            .map_err(|_| ToolSandboxError::SandboxIo)?;
...
pub fn load_mcp_call_receipts_ndjson(
    path: impl AsRef<Path>,
) -> Result<Vec<McpCallReceipt>, ToolSandboxError> {
    let path = path.as_ref();
    if !path.exists() {
        return Ok(Vec::new());
    }
    ...
    for line in reader.lines() {
        ...
        if !line.trim().is_empty() {
            receipts.push(decode_mcp_call_receipt_ndjson(&line)?);
        }
    }
    Ok(receipts)
}

pub fn verify_mcp_call_receipts(receipts: &[McpCallReceipt]) -> Result<usize, ToolSandboxError> {
...
pub fn encode_mcp_call_receipt_ndjson(receipt: &McpCallReceipt) -> String {
    let fields = [
...
pub fn decode_mcp_call_receipt_ndjson(line: &str) -> Result<McpCallReceipt, ToolSandboxError> {
...
fn expected_mcp_call_receipt_hash(receipt: &McpCallReceipt) -> u64 {
...
fn mcp_call_payload_hash(receipt: &McpCallReceipt) -> u64 {
...
impl EvidenceProducer for McpCallReceipt {
    type Record = McpCallReceipt;
    ...
    fn submission(&self) -> EvidenceSubmission {
        McpCallReceipt::submission(self)
    }
}
