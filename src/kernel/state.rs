...
impl PlanState {
        ...
        match mutation {
            ...
            PlanStatePatch::NodeRemove { node_id_hash } => {
                ...
                self.edges.retain(|edge| {
                    edge.from_node_hash != node_id_hash && edge.to_node_hash != node_id_hash
                });
            }
            PlanStatePatch::StatusChange {
                node_id_hash,
                status,
            } => {
            ...
            | PlanStatePatch::TaskLeaseRenewed {
                node_id_hash,
                worker_hash,
                claim_id,
                lease_epoch_ms,
            ...
            PlanStatePatch::TaskRetryEligible {
                node_id_hash,
                failure_claim_id: _,
                eligible_at_ms: _,
            } => {
    ...
    pub fn check_mutation(&self, mutation: PlanStatePatch) -> Result<(), PlanStateRejection> {
        match mutation {
            ...
            PlanStatePatch::NodeRemove { node_id_hash } => {
                if self.nodes.contains_key(&node_id_hash) {
                    Ok(())
                } else {
            ...
            PlanStatePatch::TaskClaim { .. }
            | PlanStatePatch::TaskLeaseRenewed { .. }
            | PlanStatePatch::TaskLeaseExpired { .. }
            | PlanStatePatch::TaskCompleted { .. }
            | PlanStatePatch::TaskFailed { .. }
            | PlanStatePatch::TaskRetryEligible { .. } => self.check_task_mutation(mutation),
        }
    }

    fn check_edge_add(&self, edge: PlanEdgeProjection) -> Result<(), PlanStateRejection> {
        if edge.from_node_hash == 0
            || edge.to_node_hash == 0
            || edge.from_node_hash == edge.to_node_hash
        {
            return Err(PlanStateRejection::InvalidEdge);
        }
        if !self.nodes.contains_key(&edge.from_node_hash)
            || !self.nodes.contains_key(&edge.to_node_hash)
        {
            return Err(PlanStateRejection::MissingNode);
        }
        Ok(())
        ...
        match mutation {
            PlanStatePatch::TaskClaim {
                ...
                if node_id_hash == 0
                    || worker_hash == 0
                    || claim_id == 0
                    || lease_expires_at_ms <= lease_epoch_ms
                {
                    return Err(PlanStateRejection::InvalidTaskLease);
                }
                let Some(node) = self.nodes.get(&node_id_hash) else {
                    return Err(PlanStateRejection::MissingNode);
                };
                if node.status != PLAN_NODE_STATUS_PENDING {
                    return Err(PlanStateRejection::TaskNotReady);
                }
                if self
                    .task_leases
                    .get(&node_id_hash)
                    .is_some_and(|lease| lease.lease_expires_at_ms > lease_epoch_ms)
                {
            ...
            PlanStatePatch::TaskLeaseRenewed {
                ...
                if lease_expires_at_ms <= lease_epoch_ms {
                    return Err(PlanStateRejection::InvalidTaskLease);
                }
                let Some(lease) = self.task_leases.get(&node_id_hash) else {
                    return Err(PlanStateRejection::MissingTaskLease);
                };
                if lease.worker_hash != worker_hash || lease.claim_id != claim_id {
                    return Err(PlanStateRejection::TaskLeaseMismatch);
                }
                if lease.lease_expires_at_ms < lease_epoch_ms {
                    return Err(PlanStateRejection::TaskLeaseExpired);
                }
                Ok(())
            }
            PlanStatePatch::NodeUpsert { .. }
            | PlanStatePatch::EdgeAdd(_)
            | PlanStatePatch::EdgeRemove(_)
            | PlanStatePatch::NodeRemove { .. }
            | PlanStatePatch::StatusChange { .. }
            | PlanStatePatch::AssigneeChange { .. }
            | PlanStatePatch::EvidenceAppend { .. }
            | PlanStatePatch::FullImport { .. }
            ...
            | PlanStatePatch::TaskCompleted { .. }
            | PlanStatePatch::TaskFailed { .. }
            | PlanStatePatch::TaskRetryEligible { .. } => self.check_task_outcome(mutation),
        }
    }

    fn check_task_terminal(
        &self,
        node_id_hash: u64,
        worker_hash: u64,
        claim_id: u64,
        ...
        if lease.lease_expires_at_ms < event_at_ms {
            return Err(PlanStateRejection::TaskLeaseExpired);
        }
        Ok(())
    }
    ...
    fn check_task_outcome(&self, mutation: PlanStatePatch) -> Result<(), PlanStateRejection> {
        match mutation {
            PlanStatePatch::TaskLeaseExpired {
                ...
                if lease.lease_expires_at_ms > expired_at_ms {
                    return Err(PlanStateRejection::TaskNotReady);
                }
                Ok(())
            }
            ...
            PlanStatePatch::TaskFailed {
                ...
                if evidence_hash == 0 || retry_after_ms < failed_at_ms {
                    return Err(PlanStateRejection::InvalidTaskEvidence);
                }
            ...
            PlanStatePatch::TaskRetryEligible {
                ...
                let Some(failure) = self.task_failures.get(&node_id_hash) else {
                    return Err(PlanStateRejection::MissingTaskFailure);
                };
                if failure.claim_id != failure_claim_id {
                    return Err(PlanStateRejection::TaskLeaseMismatch);
                }
                if failure.retry_after_ms > eligible_at_ms {
                    return Err(PlanStateRejection::TaskRetryNotEligible);
                }
                Ok(())
            }
            PlanStatePatch::NodeUpsert { .. }
            ...
            | PlanStatePatch::FullImport { .. }
            | PlanStatePatch::TaskClaim { .. }
            | PlanStatePatch::TaskLeaseRenewed { .. } => Err(PlanStateRejection::InvalidTaskLease),
        }
    }

    pub fn apply_checked_mutation(
        &mut self,
        mutation: PlanStatePatch,
    ) -> Result<u64, PlanStateRejection> {
        self.check_mutation(mutation)?;
        match mutation {
            PlanStatePatch::NodeUpsert {
                node_id_hash,
            ...
            | PlanStatePatch::TaskFailed { .. }
            | PlanStatePatch::TaskRetryEligible { .. } => {
                self.apply_checked_nonstructural_mutation(mutation)
            }
        }
