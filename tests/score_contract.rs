...
fn timing_phase_sum_adds_all_observed_work() {
    let t = PhaseTimings { upload_ms: 1, turn_total_ms: 2, candidate_scan_ms: 3, delta_download_ms: 4, delta_apply_ms: 5, validation_ms: 6, loop_total_ms: 0 };
...
fn score_values_preserve_all_goodness_axes() {
    let s = ScoreVector { intelligence: 1.0, efficiency: 0.9, correctness: 0.8, alignment: 0.7, robustness: 0.6, performance: 0.5, scalability: 0.4, determinism: 0.3, transparency: 0.2, collaboration: 0.1, empowerment: 0.9, benefit: 0.8, learning: 0.7, simplicity: 0.6, future_proofing: 0.5 };
...
fn geometric_mean_rejects_invalid_score_surfaces() {
    ...
    assert!(ScoreDelta { before: 0.80, after: 0.81 }.is_real_gain());
}

#[test]
fn recovery_classes_have_deterministic_actions() {
    assert_eq!(recovery_action(FailureClass::DirtyWorktree), RecoveryAction::RefuseUntilClean);
    assert_eq!(recovery_action(FailureClass::ManifestInvalid), RecoveryAction::StopNoMerge);
    assert_eq!(recovery_action(FailureClass::RuntimeEvidenceGap), RecoveryAction::KeepHistoryOnly);
    assert_eq!(recovery_action(FailureClass::ValidationFailure), RecoveryAction::ValidateThenReport);
}
