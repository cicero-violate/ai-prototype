...
class ObserveValidationContractTest(unittest.TestCase):
    ...
    def test_full_summary_report_mode_emits_compact_validation_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(row["policy_learning_trace_validation_result"], "pass")
            self.assertEqual(row["policy_learning_trace_status"], "pass")
            ...
            self.assertTrue(row["connector_failure_classification_present"])
            self.assertFalse(row["connector_failure_present"])
            self.assertEqual(row["connector_failure_status"], "none")
            self.assertEqual(row["connector_failure_classes"], [])
