...
The graph-derived structural score report now reports `G = 7.99 / 10` in the current local `SCORE_REPORT.md` snapshot across 18 crates. This is evidence for graph capture and structural measurement quality; it does not by itself justify changing the project-level numeric capability axes above.
