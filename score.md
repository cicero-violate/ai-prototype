...
The graph-derived structural score report now reports `G = 7.98 / 10` in the current local `SCORE_REPORT.md` snapshot across 17 crates. This is evidence for graph capture and structural measurement quality; it does not by itself justify changing the project-level numeric capability axes above.
