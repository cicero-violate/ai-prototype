...
#   --executor-count N    DAG mini-agents per scheduling wave    (default: 3, max: 5)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --executor-count) EXECUTOR_COUNT="$2"; shift 2 ;;
...
[[ -n "$EXECUTOR_COUNT" ]] && BODY+=",\"executor_count\":${EXECUTOR_COUNT}"
...
[[ -n "$EXECUTOR_COUNT" ]] && echo "  executors  : $EXECUTOR_COUNT"
