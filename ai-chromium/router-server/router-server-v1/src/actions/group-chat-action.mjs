...
export function createGroupChatActionHandler({
  parseJsonObject,
  readBody,
  errorResponse,
  jsonResponse,
  ...
  return async function handleCreateGroupChat(req, res) {
    ...
    const target = await cdpNewTarget(targetUrl);
 ...
 (async () => {
  ...
  while (Date.now() < deadline) {
    ...
    if (startGroup && !clickedStartGroup) {
      clickish(startGroup);
      clickedStartGroup = true;
      await sleep(250);
    }
    ...
    if (link) {
      return {
        ...
        group_href: link.getAttribute('href'),
      };
    }
  }
  return { ok: true, href: location.href, clicked_start_group: clickedStartGroup, group_href: null };
      ...
      await sleep(1200);
      let current = await cdp.send("Runtime.evaluate", {
      ...
      if (!groupUrl.includes("/gg/")) {
        errorResponse(res, 502, "group chat creation flow did not complete", {
