...
- Active Priorities items 19 through 21 are complete for `src/agent/prompt.rs` and graph-derived evidence refresh: the prompt helper delegation is in place, the named regression test `certification_phase_prompt_helper_preserves_distinct_phase_payloads` passes targeted and broader validation, and refreshed graph-derived structural evidence still matches the existing score rationale. Implementation step 3 found no unchecked Active Priorities item; a planning turn must select the next executable non-`root_validate` graph-backed item before another implementation turn.
