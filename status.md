...
- Active Priorities items 25 through 81 are complete in the working tree. Item 81 extracted planning-prompt context assembly from `src/agent/loop_driver.rs::LoopDriver::run_cycle(...)` into `build_project_planning_prompt(...)` while preserving spawned prompts, execute prompts, retry handling, completion errors, and inter-turn sleep behavior.
