...
- Active Priorities item 44 is complete: `ollama_effect_receipt_hash_helpers_preserve_authority_request_domains` now covers the shared Ollama effect-receipt hash helper by proving distinct authority/request hash domains, canonical effect receipt binding, valid authority-only tamper separation, valid request-only tamper separation, and invalid receipt rejection. The next executable item is item 45, the graph-derived evidence refresh and score-rationale review. `root_validate` remains explicitly non-selectable.
