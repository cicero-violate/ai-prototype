...
- Active Priorities items 25 through 75 are complete in the working tree. Item 75 extracted `wait_for_sandbox_process(...)` in `src/capability/tooling/record/process.rs`, preserving process spawn, stdout/stderr capture, hashing, sync, and receipt construction behavior while delegating only the child-process wait/timeout loop. The next planning turn should select the next graph-backed, file-scoped improvement candidate.
