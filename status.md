...
- Active Priorities items 25 through 74 are complete in the working tree. Item 74 inspected `src/capability/tooling/record/process.rs::LiveSandboxProcessExecutor::execute_authorized_process(...)` against graph-backed `SplitFn id=8d96a0680113da93` and selected the file-scoped wait-loop extraction item. The first incomplete executable item is item 75: extract `wait_for_sandbox_process(...)` in `src/capability/tooling/record/process.rs`.
