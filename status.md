...
- Active Priorities items 25 through 97 are complete in the working tree. The next executable item is item 98, extracting `src/agent/loop_driver.rs::sync_mcp_workspace(...)` TCP request/response transport into a narrow private helper. `src/agent/loop_driver.rs::run_cycle(...)` now delegates only deterministic per-turn context construction to private `build_turn_prompt_context(...)`; retry, router streaming, sleep timing, error handling, spawned-agent behavior, and MCP workspace sync remain outside the helper.
