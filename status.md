...
- Planning turn verified graph-backed prompt helper work for `src/agent/prompt.rs`: item 19 is complete in the working tree and passed targeted `cargo check --lib`; item 20 is the first unchecked executable item and must add the named prompt regression test before graph evidence refresh item 21.
