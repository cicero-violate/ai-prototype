...
- Active Priorities items 25 through 94 are complete in the working tree. The graph-backed `../chatgpt-mcp-connector/src/tools.rs::shell(...)` helper extraction now has green connector validation when tests use a workspace-local `TMPDIR`. Planning selected item 95 as the next executable item: inspect `src/agent/loop_driver.rs::run_cycle(...)` and confirm the safe prompt/label derivation helper boundary before item 96 implementation.
