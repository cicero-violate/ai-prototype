...
- Active Priorities item 43 is complete: `OllamaLlmEffectReceipt::{canonical_authority_hash, canonical_request_hash}` now share one private `fold_ordered_effect_receipt_hash(...)` helper while preserving distinct authority/request hash domains, exact field order, `is_valid()` gating, `h.max(1)` non-zero behavior, canonical effect receipt binding, replay, NDJSON, and network semantics. The next executable item is item 44, the focused regression test for authority/request hash helper domains. `root_validate` remains explicitly non-selectable.
