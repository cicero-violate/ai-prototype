...
- Active Priorities items 25 through 95 are complete in the working tree. The graph-backed `../chatgpt-mcp-connector/src/tools.rs::shell(...)` helper extraction now has green connector validation when tests use a workspace-local `TMPDIR`. Item 95 confirmed the safe `src/agent/loop_driver.rs::run_cycle(...)` helper boundary for item 96: extract only deterministic per-turn prompt/label context construction.
