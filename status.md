...
- Active Priorities items 25 through 86 are complete in the working tree. Item 86 inspected `examples/openai_tool_loop_trace.rs::submit_openai_tool_calls(...)` against graph-backed `SplitFn id=9a947eeef45104ad`; item 87 is the next unchecked item and should extract `submit_openai_process_receipt_batch(...)` in `examples/openai_tool_loop_trace.rs` only.
