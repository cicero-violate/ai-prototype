...
- Active Priorities items 19 and 20 are complete for `src/agent/prompt.rs`: the prompt helper delegation is in place and the named regression test `certification_phase_prompt_helper_preserves_distinct_phase_payloads` passes targeted and broader validation. Item 21 is the first unchecked item and must refresh graph-derived structural evidence before score review.
