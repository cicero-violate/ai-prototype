...
- Active Priorities items 43 through 45 are now planned for `src/capability/llm/ollama.rs`, `src/lib.rs`, and graph-derived evidence refresh. The first executable item is item 43, which must route `OllamaLlmEffectReceipt::{canonical_authority_hash, canonical_request_hash}` through one private ordered hash-fold helper while preserving distinct authority/request hash domains, field order, `is_valid()` gating, non-zero behavior, canonical effect receipt binding, replay, NDJSON, and network semantics. `root_validate` remains explicitly non-selectable.
