...
- First incomplete Active Priorities item after implementation step 1 on 2026-05-10: `src/domain/scoring.rs` unit test `verdict_research_thresholds` item 13.
...
- `src/domain/scoring.rs` currently contains validated `BoundedScore` helpers, score-input breakdown helpers, conservative `verdict_for_scores(...)`, and passing `verdict_ignore_thresholds` plus `verdict_watch_thresholds`; remaining explicit verdict threshold tests start at `verdict_research_thresholds`.
