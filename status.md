...
- Active Priorities items 25 through 85 are complete in the working tree. Item 86 is the next unchecked item: inspect `examples/openai_tool_loop_trace.rs::submit_openai_tool_calls(...)` against graph-backed `SplitFn id=9a947eeef45104ad` and validate with `cargo check --example openai_tool_loop_trace`.
