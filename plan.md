...
58. [ ] `src/agent/loop_driver.rs::LoopDriver::run_cycle`: extract exactly the retry-attempt body into `run_cycle_attempt(...)` while preserving `LoopDriver::run_cycle(...)` public behavior and signature.
   - Scope: `src/agent/loop_driver.rs` only. Add a private helper near `run_cycle` that accepts the current attempt context (`tag`, `cycle_num`, `label`, `prompt`, `turn_mode`, retry attempt number, `RouterClient`, and timeout/logger config fields as needed), creates `ChunkLogger`, sends `OpenAiChatRequest`, updates/returns the last reason, and reports whether the turn completed. Do not edit `sync_mcp_workspace`, graph mutation code, domain code, tests, fixtures, or generated auto-refactor JSON.
   - Done when: `run_cycle` still owns total-turn computation, spawned/planning/execute prompt selection, sleep/retry loop control, completion error handling, router-tab close, and certification; only the per-attempt streaming/logger block from the inner retry loop is delegated to the helper. The helper name is `run_cycle_attempt`; the original `run_cycle` signature remains unchanged.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
