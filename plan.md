...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json`, and the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains a weak graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten.

The selected graph-editor plan is schema version 1 and contains 1,614 planned operations: one already-completed `SplitFn` candidate for `agent::loop_driver::LoopDriver::run_cycle` plus 1,613 `MergeFns` candidates. The previous local `ai` crate candidate in `src/api/server.rs` is complete. Fresh selectable work starts with graph operation id `e82fccc0c01ca02f`, a `MergeFns` candidate for `agent::config::{env_u32, env_u64}`. This is a small, local parser-surface consolidation that can improve Structure/Simplicity evidence without touching `root_validate`, kernel state-machine semantics, transport ledgers, route DTOs, or capability policy. Generated canonical names are evidence, not mandatory names; implementation must preserve existing environment-variable behavior and accepted defaults.

1. [x] `src/agent/config.rs`: replace the duplicate `env_u32(...)` and `env_u64(...)` wrappers with direct typed uses of `env_parsed::<u32>(...)` and `env_parsed::<u64>(...)` in `AgentLoopConfig::from_env(...)`.
   - Scope: `src/agent/config.rs` function `AgentLoopConfig::from_env(...)` and private helpers `env_u32(...)`, `env_u64(...)`, `env_parsed(...)` only.
