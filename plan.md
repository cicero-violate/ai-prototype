...
Planning reconnaissance on 2026-05-12 confirms items 60 through 91 are complete with targeted and broader validation as recorded below. The next executable item is item 94, validation completion for the already-applied `../chatgpt-mcp-connector/src/tools.rs::shell(...)` helper extraction now that item 93 restored `../chatgpt-mcp-connector/src/mcp_guard.rs::ai_worker_port()` to environment-only behavior. Current `SplitFn id=ce8d33f07ffdaae6` for `tools::shell` remains legitimate graph-backed evidence, and the scoped `tools.rs` helper extraction is already present in the sibling connector working tree. The prior validation blocker was an unrelated dirty `src/mcp_guard.rs` change that defaulted `AI_WORKER_PORT` to `9100`; item 93 restored env-only behavior and connector `cargo check` now passes. Generated `merge_surface` operations remain deferred and must be filtered manually.
