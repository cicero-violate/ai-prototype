...
Graph-derived score evidence in the local `SCORE_REPORT.md` on 2026-05-13 reports schema version 16, 14 crates, `G = 7.96 / 10`, Architecture `8.9`, Structure `4.8`, Simplicity `7.2`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`. The local `ai` crate row reports 5,472 nodes, 35,235 edges, 2,208 functions, Architecture `9.5`, Structure `6.0`, Simplicity `7.0`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; the local `root_validate` row reports 162 nodes, 1,971 edges, 160 functions, Architecture `3.2`, Structure `1.5`, Simplicity `6.2`, Maintainability `10.0`, Determinism `10.0`, and Coherency `7.3`. This refresh does not justify a project-level numeric score increase by itself. Item 148 completed the durable caller-selected evaluator helper-boundary inspection. Item 149 documented the source-availability blocker for `../chatgpt-mcp-connector/src/tools.rs` and `../chatgpt-mcp-connector/Cargo.toml`; the connector extraction remains deferred until the sibling source tree is exposed.
