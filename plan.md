...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; listed current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`; inspected `src/api/transport.rs` and `tests/api_transport_contract.rs`; and checked the working tree. There was no first incomplete executable item before this planning turn: Active Priorities items 43 through 45 were complete and `status.md` confirmed checklist exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence remains `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan remains schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `capability::EvidenceSubmission::{new, with_effect}`, `capability::eval::record::{dimension_order_hash, dimension_score_hash}`, `capability::judgment::record::PolicyJudgmentRecord::{decision, judgment_record}`, `capability::judgment::record::PolicyReuseCostCatalogReceipt::{from_source_hashes, is_valid}`, `capability::learning::promote::<impl PolicyStore>::{promote, promote_feedback, promote_durable, promote_feedback_durable}`, and `graph_mutation::GraphMutationOpRow::{is_self_consistent, new}`. `agent::loop_driver::LoopDriver::run_cycle` split operation `001e821dc83e940a` remains rejected for this turn because `src/agent/loop_driver.rs` already has unrelated uncommitted edits. Adjacent `agent::cycle` route/hash wrapper candidates remain rejected because current source already has shared route/hash helpers or because structurally similar wrappers represent distinct semantic route domains. Judgment hash, policy-reuse receipt hash, LLM role-message, receipt encoder, codec enum converter, domain bridge target, and graph mutation encoder candidates remain rejected for this turn because structurally similar wrappers represent distinct semantic domains or record families.

Graph operation `68bee1c8c928f60c` identifies `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}` as the next safe non-root consolidation area. The safe subset is lookup delegation only: `has_conflicting_request(...)` may route through the existing receipt lookup boundary used by `contains_request_id(...)`, and a private helper may be added if it makes the relationship explicit. Do not collapse the two public methods into one semantic API: `contains_request_id(...)` is a request-presence predicate, while `has_conflicting_request(...)` is a payload-hash conflict predicate over an `ApiTransportFrame`. Preserve all public signatures, error variants, replay behavior, receipt validation behavior, frame validation behavior, command handling, and serialization.

46. [x] `src/api/transport.rs`: route `ApiTransportLedger::has_conflicting_request(...)` through the same receipt lookup boundary as `ApiTransportLedger::contains_request_id(...)`.
   - Scope: `src/api/transport.rs` only; allowed functions are `ApiTransportLedger::contains_request_id(...)`, `ApiTransportLedger::has_conflicting_request(...)`, and at most one new private helper on `ApiTransportLedger` if needed. Do not change `ApiTransportFrame`, `ApiTransportReceipt`, `handle_transport_frame_once(...)`, `push_receipt(...)`, public method signatures, public struct fields, command matching, error variants, replay paths, or any tests in this item.
   - Done when: duplicate request detection and same-request/different-payload conflict detection still use the same receipt lookup source, `contains_request_id(...)` remains true for any existing request id, `has_conflicting_request(...)` remains true only when an existing request id has a different payload hash, and no replay or receipt-contract behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

47. [x] `tests/api_transport_contract.rs` test `transport_ledger_conflicting_request_detection_is_payload_hash_scoped`: strengthen conflict-boundary regression coverage.
   - Scope: `tests/api_transport_contract.rs` only; use existing API transport fixture helpers and the existing `ApiTransportLedger` public API. Do not change source code in this item.
   - Done when: the named test proves a stored receipt makes `contains_request_id(request_id)` true, leaves `contains_request_id(other_request_id)` false, leaves `has_conflicting_request(&same_request_same_payload_frame)` false, makes `has_conflicting_request(&same_request_different_payload_frame)` true, and leaves `has_conflicting_request(&different_request_same_payload_frame)` false. The test must not perform network I/O, filesystem I/O outside normal cargo test execution, environment mutation, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --test api_transport_contract transport_ledger_conflicting_request_detection_is_payload_hash_scoped -- --test-threads=1`.

48. [ ] `SCORE_REPORT.md`: after items 46 and 47 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
