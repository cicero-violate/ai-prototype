...
Planning reconnaissance on 2026-05-13 confirms items 60 through 142 are complete with targeted validation, reconciliation evidence, or graph-derived score refresh evidence as recorded below. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next executable work should prefer small, graph-backed decomposition with validation evidence. The first incomplete Active Priorities item is item 143, `../chatgpt-mcp-connector/src/tools.rs` extraction for the graph-backed `tools::execute_evaluator_suite_tool_inner(...)` split candidate from `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`. Item 142 confirmed `SplitFn id=a57f325e92fb8f3f` still targets `tools::execute_evaluator_suite_tool_inner(...)` at expected range `21778..26627`; generated names `execute_evaluator_suite_tool_inner__parse` and `execute_evaluator_suite_tool_inner__transform` are rejected as direct instructions. The item-143 manual boundary is one private helper in `../chatgpt-mcp-connector/src/tools.rs` that owns caller-selected evaluator suite resolution, timeout/max-output/run/tlog/candidate/completion argument parsing, allowed-program parsing, max-score derivation, and `EvaluatorCommandPolicy` construction, while leaving workspace/cwd resolution, `CapabilityRequest` construction, run/actor creation, tlog-path branching, and durable/in-memory execution dispatch in `execute_evaluator_suite_tool_inner(...)`. The following legitimate candidate after item 144 is `tools::execute_durable_policy_selected_suite(...)`. Do not apply generated names or merge-surface recommendations blindly.
...
143. [ ] `../chatgpt-mcp-connector/src/tools.rs`: extract one private helper from `execute_evaluator_suite_tool_inner(...)` for caller-selected evaluator transport input parsing and command-policy construction only.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs::execute_evaluator_suite_tool_inner(...)` and one private helper in the same file selected by item 142. Do not change required `suite` validation, workspace/cwd resolution, `CapabilityRequest` construction, run id creation, actor id creation, durable-vs-in-memory branching, evaluator response shape, TLog persistence semantics, or public error semantics.
   - Done when: `execute_evaluator_suite_tool_inner(...)` still owns workspace/cwd resolution, request construction, run/actor creation, tlog-path branching, and execution dispatch, while delegating only suite parsing/resolution, timeout/max-output/run/tlog/candidate/completion argument parsing, allowed-program parsing, max-score derivation, and `EvaluatorCommandPolicy` construction to a private helper returning resolved caller-selected evaluator transport inputs.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_evaluator_suite -- --test-threads=1`.

144. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 143 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
