...
64. [ ] `src/agent/cycle.rs`: inspect graph-backed `SplitFn` evidence for `agent::cycle::AgentCycle::run` and write the smallest helper-extraction implementation checklist item for that exact function.
   ...
   - Validation: inspect `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` for `fn_path=agent::cycle::AgentCycle::run`; inspect `src/agent/cycle.rs`; update `plan.md` and `status.md` with the next file-scoped implementation item.
