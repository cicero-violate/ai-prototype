...
Planning reconnaissance on 2026-05-12 confirms items 60 through 69 are complete with targeted and broader validation as recorded below. The first incomplete executable item is item 70, `src/runtime/introspection.rs::introspect_canonical_tlog(...)`. Python parsing of `state/rustc/auto-refactor/*.graph-editor-plan.json` reconfirmed current graph-backed `SplitFn id=d2051ab2f7cd8a57` for `runtime::introspection::introspect_canonical_tlog`; it is smaller and lower-risk than `graph_mutation::generate_graph_patch`, `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process`, or connector/tool split candidates, and it directly improves evidence-transparency structure while Structure remains the lowest graph-derived axis in `SCORE_REPORT.md`. Source reconnaissance found the smallest safe helper boundary is the canonical evidence line scan inside `introspect_canonical_tlog(...)`, leaving `load_tlog_ndjson(path)`, latest phase/event counting, canonical path assignment, and worker-state detection in the public function.
...
70. [ ] `src/runtime/introspection.rs`: extract canonical evidence-line scanning from `introspect_canonical_tlog(path)` into private helper `scan_canonical_tlog_evidence(path: &Path) -> Result<CanonicalEvidenceScan, CanonError>` while preserving `introspect_canonical_tlog(path: impl AsRef<Path>) -> Result<CanonicalIntrospectionReport, CanonError>`.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --test canonical_tlog_contract -- --test-threads=1 && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
