...
112. [ ] `src/agent/cycle.rs`: extract private helper `dispatch_observed_phase(...)` from the `match phase.as_str()` block inside `AgentCycle::run(...)` without changing run-loop semantics.
