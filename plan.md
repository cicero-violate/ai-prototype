...
Planning reconnaissance on 2026-05-12 confirms items 60 through 83 are complete with targeted and broader validation as recorded below. The next executable item is item 84, a graph-backed inspection of the remaining repeated LLM phase dispatch pattern in `src/agent/cycle.rs::AgentCycle::run(...)`. Item 83 extracted `finalize_streaming_response(...)` from `src/agent/router.rs::send_streaming_request(...)` with passing focused router streaming validation, `cargo fmt --check`, and full-suite validation. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` remains legitimate graph-backed evidence because `AgentCycle::run(...)` still contains repeated `Analysis`, `Judgment`, `Plan`, and `Eval` LLM-turn branches that each perform phase prompt dispatch, sentinel detection, verdict parsing, and phase-gate evidence submission. The proposed next helper boundary is this repeated LLM phase branch only; do not move recovery, invariant, execute, verify, persist/learn, loop termination, worker observation, final-summary construction, or kernel authority into the helper. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
84. [ ] `src/agent/cycle.rs`: inspect graph-backed `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and record the smallest safe helper-extraction checklist item for the repeated LLM phase branch in that exact function.
