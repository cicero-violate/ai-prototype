...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and the source/test surfaces named below. There was no first incomplete executable item before this planning turn: prior Active Priorities items 19 through 21 were complete and `status.md` confirmed item exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle` phase-route/recovery-route helper tables, `agent::objective` hash setters, `agent::prompt` certification prompt formatting, `api::server` submission token parsing, `agent::cycle` hash-domain wrapper extraction plus descriptor regression coverage, `agent::router::{env_u32, env_u64}` retry parser consolidation plus parser regression coverage, `agent::worker_client::{WorkerClient::new, WorkerClient::new_with_timeout}` constructor delegation plus constructor regression coverage, `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}` request-id lookup delegation plus direct conflict regression coverage, `api::transport::ApiTransportReceipt::{new, is_contract_valid}` expected-hash helper coverage, `api::transport::ApiTransportSession::{from_parts, verify}` shared verifier plus direct tampered-receipt session coverage, and `capability::CapabilityEffectRoute::{allows, permits_effect}` passed-submission delegation plus failed-submission regression coverage. Several remaining `agent::cycle`, prompt, evidence, transport hash, and receipt encoder candidates are structurally similar but semantically unsafe to merge wholesale because the names document distinct envelope, contract, command, gate, evidence, effect, phase, prompt, transport-frame, graph-receipt, and receipt-hash domains.

Graph operation `4906e7371d5af9ed` identifies `graph_mutation::GraphMutationOpRow::{is_self_consistent, new}` as the next safe non-root consolidation area because both rely on the same schema-version, record-type, op-hash, and expected-row-hash invariant. The safe subset is to extract a private row-field validator used by `is_self_consistent(...)`, while preserving `GraphMutationOpRow::new(...)` as the only constructor that sets `row_hash = expected_row_hash()`. Existing graph mutation tests cover NDJSON round trips and tampered row hashes; the plan adds direct row-level coverage for valid constructor output and each invalid field before refreshing graph evidence.

22. [x] `src/graph_mutation.rs`: extract a private `GraphMutationOpRow` invariant helper and delegate `GraphMutationOpRow::is_self_consistent(...)` through it.
   - Scope: `src/graph_mutation.rs` `impl GraphMutationOpRow` only; allowed functions are `GraphMutationOpRow::is_self_consistent(...)` and a private local helper in the same impl. Do not change `GraphMutationOpRow::new(...)`, `GraphMutationOpRow::expected_row_hash(...)`, public fields, graph mutation schema constants, NDJSON encoders/decoders, verifier behavior, or receipt hash semantics.
