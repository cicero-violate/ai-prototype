...
Graph operations `76de2626ddf92d55`, `60f6ec4427231d32`, and `6f2ca76087786cf6` identify `capability::eval::record::{dimension_order_hash, dimension_score_hash, eval_payload_hash}` as the next safe non-root consolidation area. The safe subset is not to merge these public semantic hash domains. Instead, extract the repeated dimension iteration into a private helper that accepts the domain seed and an explicit `DimensionHashMode`, then keep `dimension_order_hash(...)` and `dimension_score_hash(...)` as thin wrappers with distinct seeds and distinct score/threshold inclusion semantics. `eval_payload_hash(...)` must remain a full eval-payload hash because it includes record score and threshold as well as dimension content. Existing tests cover payload binding and dimension-order tamper rejection; the plan adds direct wrapper-domain regression coverage before refreshing graph evidence.
