...
Graph-derived score evidence in the local `SCORE_REPORT.md` now reports schema version 16, 2 crates, `G = 8.05 / 10`, Architecture `9.3`, Structure `5.0`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`. The local `ai` crate row reports 5,472 nodes, 35,235 edges, 2,208 functions, Architecture `9.5`, Structure `6.0`, Simplicity `7.0`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; the local `chatgpt_mcp_connector` row reports 3,714 nodes, 21,661 edges, 1,704 functions, Architecture `9.0`, Structure `3.6`, Simplicity `7.4`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.3`. This refresh does not justify a project-level numeric score increase by itself. Item 148 completed the durable caller-selected evaluator helper-boundary inspection. Item 149 documented the source-availability blocker for `../chatgpt-mcp-connector/src/tools.rs` and `../chatgpt-mcp-connector/Cargo.toml`; the connector extraction remains deferred until the sibling source tree is exposed.
...
159. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 158 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
