...
Planning reconnaissance on 2026-05-13 confirms items 60 through 149 are complete with targeted validation, reconciliation evidence, graph-derived score refresh evidence, graph-backed inspection evidence, or explicit blocker evidence as recorded below. Local `SCORE_REPORT.md` currently reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next executable work should prefer small decomposition or focused test evidence. Item 149 documented that the sibling connector source path required by the prior item-150 extraction is not exposed from this workspace: `test -f ../chatgpt-mcp-connector/src/tools.rs && test -f ../chatgpt-mcp-connector/Cargo.toml` returned `source_availability_rc=1`, and `ls` reported `../chatgpt-mcp-connector` does not exist. The durable caller-selected evaluator extraction is deferred until that source tree is mounted. Current local auto-refactor plans expose no safe local `SplitFn` entries for the `ai` crate; `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` contains merge-surface operations only, which remain rejected as direct implementation instructions. The next executable work is item 152, a local source-backed inspection of `src/bin/root_validate.rs`, because `SCORE_REPORT.md` shows `root_validate` has the lowest graph-derived Structure score at `1.5` among local crates. Do not apply generated merge-surface recommendations blindly.
...
152. [ ] `src/bin/root_validate.rs`: inspect the compact-mode dispatch/catalog boundary and record the exact smallest follow-on test or helper boundary.
...
153. [ ] `src/bin/root_validate.rs`: add the item-152-approved focused test or one private helper for the compact-mode dispatch/catalog boundary.
   - Scope: one focused `src/bin/root_validate.rs` test or one private helper in the same file selected by item 152. Do not change compact mode names, markers, exit codes, `mirror_validation_outcome(...)`, full root validation fallback behavior, or JSON record schemas.
   - Done when: the selected boundary is implemented and targeted root-validate validation passes.
   - Validation: `cargo test --bin root_validate -- --test-threads=1`.
