...
Planning reconnaissance on 2026-05-12 confirms items 60 through 91 are complete with targeted and broader validation as recorded below. The next executable item is item 92, a graph-backed inspection of `../chatgpt-mcp-connector/src/tools.rs::shell(...)`. Current `SplitFn id=ce8d33f07ffdaae6` for `tools::shell` is legitimate graph-backed evidence because the function still combines argument parsing, workspace/cwd resolution, shell process spawn, bounded stdout/stderr pipe tasks, timeout/kill handling, task joins, output rendering, and MCP JSON response construction in one function. The proposed next helper boundary should be selected by inspection before editing source; generated `merge_surface` operations remain deferred and must be filtered manually.
...
92. [ ] `../chatgpt-mcp-connector/src/tools.rs`: inspect graph-backed `SplitFn id=ce8d33f07ffdaae6` for `tools::shell` and record the smallest safe helper-extraction checklist item for that exact function.
...
93. [ ] `../chatgpt-mcp-connector/src/tools.rs`: extract only the item-92-approved boundary from `shell(args, workspace)` into a private helper while preserving the public MCP shell tool behavior.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs` only.
   - Done when: `shell(...)` still validates `command`, `timeout_ms`, `max_output_bytes`, resolves `cwd` through `WorkspaceConfig`, spawns `/bin/sh -c` in the resolved workspace directory, preserves bounded stdout/stderr capture, timeout/kill behavior, exit metadata, truncation metadata, and JSON response fields, and delegates only the approved low-risk sub-block to the helper.
