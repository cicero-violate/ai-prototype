...
40. [ ] `src/agent/loop_driver.rs`: split `LoopDriver::run_cycle(...)` by extracting one private cycle-preparation helper for spawned/project mode turn scheduling.
...
41. [ ] `src/agent/loop_driver.rs` test `run_cycle_preparation_preserves_project_and_spawned_turn_schedules`: add focused regression coverage for the extracted cycle-preparation helper.
...
42. [ ] `SCORE_REPORT.md`: after items 40 and 41 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
