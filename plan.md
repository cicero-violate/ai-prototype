...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and the source/test surfaces named below. There was no first incomplete executable item before this planning turn: prior Active Priorities items 22 through 24 were complete and `status.md` confirmed item exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle` phase-route/recovery-route helper tables, `agent::objective` hash setters, `agent::prompt` certification prompt formatting, `api::server` submission token parsing, `agent::cycle` hash-domain wrapper extraction plus descriptor regression coverage, `agent::router::{env_u32, env_u64}` retry parser consolidation plus parser regression coverage, `agent::worker_client::{WorkerClient::new, WorkerClient::new_with_timeout}` constructor delegation plus constructor regression coverage, `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}` request-id lookup delegation plus direct conflict regression coverage, `api::transport::ApiTransportReceipt::{new, is_contract_valid}` expected-hash helper coverage, `api::transport::ApiTransportSession::{from_parts, verify}` shared verifier plus direct tampered-receipt session coverage, `capability::CapabilityEffectRoute::{allows, permits_effect}` passed-submission delegation plus failed-submission regression coverage, and `graph_mutation::GraphMutationOpRow::{is_self_consistent, new}` row invariant helper plus tampered-field regression coverage. Several remaining `agent::cycle`, prompt, evidence, transport hash, receipt encoder, and policy-reuse hash candidates are structurally similar but semantically unsafe to merge wholesale because the names document distinct envelope, contract, command, gate, evidence, effect, phase, prompt, transport-frame, graph-receipt, policy, trend, and receipt-hash domains.

Graph operation `e854b50618421884` identifies `capability::EvidenceSubmission::{new, with_effect}` as the next safe non-root consolidation area because both constructors create the same gate/evidence/passed/effect/payload record shape and differ only in whether the effect is explicitly supplied or defaulted to `PacketEffect::None`. The safe subset is to make `EvidenceSubmission::new(...)` delegate through `EvidenceSubmission::with_effect(...)`, while preserving `with_payload(...)` as the explicit payload constructor and preserving `with_effect_payload(...)` as the final field initializer. Existing capability tests cover route semantics; the plan adds direct constructor equivalence and contract coverage before refreshing graph evidence.

25. [x] `src/capability/mod.rs`: delegate `EvidenceSubmission::new(...)` through `EvidenceSubmission::with_effect(...)`.
   - Scope: `src/capability/mod.rs` `impl EvidenceSubmission` only; allowed function is `EvidenceSubmission::new(...)`. Do not change `EvidenceSubmission::with_payload(...)`, `EvidenceSubmission::with_effect(...)`, `EvidenceSubmission::with_effect_payload(...)`, `EvidenceSubmission::is_contract_valid(...)`, `structural_payload_hash(...)`, public fields, enum variants, route constants, registry behavior, packet effect semantics, or public signatures.
