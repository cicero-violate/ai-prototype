...
Planning reconnaissance on 2026-05-12 confirms items 60 through 103 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 104, focused in-memory success coverage for `finalize_streaming_response(...)`, followed by in-memory error coverage in item 105 and graph-derived evidence refresh in item 106. Current `SplitFn id=c07f9b3fef3c6e37` for `agent::router::collect_streaming_response_bytes` (`expected_lo=16668`, `expected_hi=19103`, generated names `collect_streaming_response_bytes__parse`/`collect_streaming_response_bytes__transform`) is the live graph-backed follow-on candidate after item 102 extracted the helper; it is evidence only, not an instruction to split again before finalization tests. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only. The legitimate current source target is `src/agent/router.rs::finalize_streaming_response(...)` test coverage, because Structure remains the lowest graph-derived axis and tests around the new transport/finalization boundary provide validation evidence before any further graph-backed refactor.
...
104. [ ] `src/agent/router.rs`: add in-memory success coverage for `finalize_streaming_response(...)` using a 200 HTTP response with SSE content and a terminating `data: [DONE]` frame.
