...
47. [ ] `tests/test_domain_fixture_contract.py::test_fixtures_include_expected_risk_result`: add a unittest that iterates all `REQUIRED_FIXTURES`, asserts top-level `expected_risk_result` exists, asserts it is either `"pass"` or `"block"`, and asserts it equals `expected_risk_envelope["expected_result"]`.
   ...
   - Validation: `python3 -m unittest tests.test_domain_fixture_contract.DomainFixtureContract.test_fixtures_include_expected_risk_result`.
48. [ ] `tests/test_domain_fixture_contract.py::test_required_fixture_fields_have_clear_assertions`: add a unittest that iterates all `REQUIRED_FIXTURES` and reports missing required top-level fixture fields with the fixture name and field name.
...
49. [ ] `tests/test_domain_fixture_contract.py`: run the full fixture contract after item 47 and item 48.
