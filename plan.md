...
Graph-derived score evidence from `SCORE_REPORT.md` was refreshed on 2026-05-13 after item 146: schema version 16, 16 crates, `G = 8.02 / 10`, Architecture `8.9`, Structure `4.8`, Simplicity `7.4`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.5`. The refreshed `ai` crate row reports 5,474 nodes, 33,942 edges, 2,210 functions, Architecture `9.3`, Structure `6.0`, Simplicity `7.6`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.6`. This refresh does not justify a project-level numeric score increase by itself.
...
148. [ ] `../chatgpt-mcp-connector/src/tools.rs`: inspect live graph-backed `execute_durable_selected_suite(...)` and record the exact helper boundary for the next execution item.
