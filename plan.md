...
Planning reconnaissance on 2026-05-12 confirms items 60 through 73 are complete with targeted and broader validation as recorded below. The first incomplete executable item is item 74, a graph-backed inspection item for `src/capability/tooling/record/process.rs::LiveSandboxProcessExecutor::execute_authorized_process(...)`. The workspace is clean before this planning update. Current auto-refactor reconnaissance parsed `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` and confirms `SplitFn id=8d96a0680113da93` for `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process` with `expected_lo=6609`, `expected_hi=8939`, generated names `execute_authorized_process__parse` and `execute_authorized_process__transform`, and `delegate_strategy=preserve_original_signature`. Source inspection found the smallest safe helper boundary is the timeout/try-wait loop inside `execute_authorized_process(...)`, with existing sandbox process receipt tests available through `cargo test live_sandbox_process_runner_records_and_replays_receipt -- --test-threads=1`. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
74. [ ] `src/capability/tooling/record/process.rs`: inspect graph-backed `SplitFn id=8d96a0680113da93` for `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process` and write the smallest helper-extraction implementation checklist item for that exact function.
...
75. [ ] `src/capability/tooling/record/process.rs`: extract the child-process timeout wait loop from `execute_authorized_process(...)` into private helper `wait_for_sandbox_process(child: &mut std::process::Child, timeout: Duration) -> Result<(u64, bool), ToolSandboxError>` while preserving `LiveSandboxProcessExecutor::execute_authorized_process(plan) -> Result<SandboxProcessEffect, ToolSandboxError>` behavior.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test live_sandbox_process_runner_records_and_replays_receipt -- --test-threads=1 && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
