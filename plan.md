...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; inspected `../state/rustc/auto-refactor/*.graph-editor-plan.json`; parsed the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`; inspected candidate files `src/agent/prompt.rs`, `src/agent/objective.rs`, `src/domain/scoring.rs`, `src/domain/bridge.rs`, and `src/capability/llm/ollama.rs`; and checked the working tree. There was no first incomplete executable item before this planning turn: Active Priorities items 4 through 6 were complete and `status.md` confirmed checklist exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

Current graph-derived aggregate evidence remains `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan remains schema version 1, graph schema version 16, and contains 1,614 planned operations. Recently reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `agent::objective::{with_risk_envelope, with_stop_condition}`, `capability::llm::ollama::OllamaConfig::{base_url_id, model_id}`, and related graph-backed helper candidates. `agent::loop_driver` candidates remain rejected for this turn because `src/agent/loop_driver.rs` has unrelated uncommitted edits. Graph candidates that merge semantically distinct hash, route, bridge, score, receipt, or record-family functions remain rejected unless the execute turn preserves their public semantic APIs and consolidates only private construction mechanics.

Graph operation `64cfcb1a6f89964d` identifies `capability::llm::ollama::OllamaMessage::{system, user}` as the next safe non-root consolidation area. The safe subset is constructor delegation only: both public constructors may route through one private role/content helper. Do not collapse the public constructors into one semantic API: `system(...)` is the system-message boundary, while `user(...)` is the user-message boundary. Preserve public signatures, role strings, content ownership behavior, request JSON behavior, message ordering, context-to-message behavior, network/client behavior, receipt/proof behavior, NDJSON behavior, and all existing config/retry semantics.

7. [x] `src/capability/llm/ollama.rs`: route `OllamaMessage::system(...)` and `OllamaMessage::user(...)` through one private message-constructor helper.
   - Scope: `src/capability/llm/ollama.rs` only; allowed functions are `OllamaMessage::system(...)`, `OllamaMessage::user(...)`, and at most one new private helper near `OllamaMessage`. Do not change `OllamaConfig`, `OllamaClient`, request serialization, retry-budget code, HTTP/client code, receipt/proof structs, NDJSON code, `messages_from_context(...)`, or tests in this item.
