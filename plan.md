...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; parsed the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`; inspected candidate source and test surfaces in `src/capability/llm/openai.rs`; and checked the working tree. There was no first incomplete executable item before this planning turn: Active Priorities items 7 through 9 were complete and `status.md` confirmed checklist exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

Current graph-derived aggregate evidence remains `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan remains schema version 1, graph schema version 16, and contains 1,614 planned operations. Recently reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `agent::objective::{with_risk_envelope, with_stop_condition}`, `capability::llm::ollama::OllamaConfig::{base_url_id, model_id}`, `capability::llm::ollama::OllamaMessage::{system, user}`, and related graph-backed helper candidates. `agent::loop_driver` candidates remain rejected for this turn because `src/agent/loop_driver.rs` has unrelated uncommitted edits. Graph candidates that merge semantically distinct hash, route, bridge, score, receipt, or record-family functions remain rejected unless the execute turn preserves their public semantic APIs and consolidates only private construction mechanics.

Graph operation `9f1d902307750fc6` identifies `capability::llm::openai::OpenAiConfig::{base_url_id, model_id}` as the next safe non-root consolidation area. The safe subset is private text-id helper delegation only: both public methods may route through one private helper that delegates to `provider_text_hash(...)`. Do not collapse the public methods into one semantic API: `model_id(...)` is the model identity boundary, while `base_url_id(...)` is the endpoint identity boundary. Preserve public signatures, field selection, environment parsing, config validation, local-endpoint validation, chat-completions path behavior, client construction, request JSON behavior, retry-budget behavior, receipt/proof behavior, NDJSON behavior, and all network surfaces.

10. [x] `src/capability/llm/openai.rs`: route `OpenAiConfig::model_id(...)` and `OpenAiConfig::base_url_id(...)` through one private text-id helper.
   - Scope: `src/capability/llm/openai.rs` only; allowed functions are `OpenAiConfig::model_id(...)`, `OpenAiConfig::base_url_id(...)`, and at most one new private helper near `OpenAiConfig`. Do not change `OpenAiConfig::from_env(...)`, `OpenAiConfig::validate(...)`, `OpenAiConfig::chat_completions_path(...)`, `OpenAiRetryBudgetPolicy`, `OpenAiClient`, request serialization, HTTP/client code, receipt/proof structs, NDJSON code, `messages_from_context(...)`, `OpenAiMessage`, `OpenAiFunctionTool`, or tests in this item.
   - Done when: `OpenAiConfig::model_id(...)` still derives from `self.model`, `OpenAiConfig::base_url_id(...)` still derives from `self.base_url`, both preserve existing hash values through `provider_text_hash(...)`, and both public methods delegate to the same private helper boundary.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

11. [x] `src/capability/llm/openai.rs` test `openai_config_id_helpers_preserve_distinct_field_boundaries`: add regression coverage for the shared text-id helper.
   - Scope: `src/capability/llm/openai.rs` test module only; use existing `OpenAiConfig` public API. Do not change production code in this item.
