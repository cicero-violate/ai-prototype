...
Planning reconnaissance on 2026-05-12 confirms that item 58 is complete: `status.md` records green `cargo fmt --check` plus full-suite `cargo test --all-targets` evidence for the existing `src/agent/loop_driver.rs::LoopDriver::run_cycle_attempt(...)` extraction. The first incomplete executable item is now item 60, `src/agent/loop_driver.rs::sync_mcp_workspace` helper extraction. Source inspection found `sync_mcp_workspace` at `src/agent/loop_driver.rs:700`, request construction around line 724, status parsing around line 736, and tests beginning around line 887. The graph-backed `SplitFn id=d535999f445621fb` targets `agent::loop_driver::sync_mcp_workspace` with generated names `sync_mcp_workspace__parse` and `sync_mcp_workspace__transform`; use it only as evidence for a small manual extraction. The graph-derived objective still favors small helper extractions because `SCORE_REPORT.md` reports Structure as the lowest aggregate structural axis at `4.8`. Do not advance to `AgentCycle::run`, router, graph mutation, or process-executor split candidates until item 60 has green validation evidence or a new blocker entry explains why it cannot be completed.
...
60. [ ] `src/agent/loop_driver.rs`: extract URL parsing from `sync_mcp_workspace` into private helper `parse_mcp_workspace_endpoint(mcp_url: &str) -> Result<(String, u16), String>` while preserving `sync_mcp_workspace(mcp_url: &str, project_dir: &Path) -> Result<(), String>`.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test loop_driver::tests::mcp_workspace_endpoint_parser_preserves_host_port_and_errors -- --test-threads=1`, then `cargo fmt --check`, then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
61. [ ] `src/agent/loop_driver.rs`: extract request-body construction from `sync_mcp_workspace` into private helper `build_mcp_workspace_request(host: &str, port: u16, project_dir: &Path) -> String` after item 60 is green.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test loop_driver::tests::mcp_workspace_request_builder_preserves_workspace_post -- --test-threads=1`, then `cargo fmt --check`, then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
