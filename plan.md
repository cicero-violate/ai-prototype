...
Graph operation `e5d5eb5627204595` identifies `capability::llm::openai::OpenAiFunctionTool::{new, with_description}` as the next safe non-root consolidation area. The safe subset is constructor delegation only: `OpenAiFunctionTool::new(...)` may route through a private constructor helper shared with `with_description(...)`, while preserving the current public signatures, field values, `description: None` default, parameter JSON handling, request serialization, tool contract validation, router behavior, retry-budget behavior, NDJSON receipt behavior, and all network surfaces. `src/capability/llm/openai.rs` currently has no local `#[cfg(test)]` module; add only focused in-file constructor regression coverage.

43. [x] `src/capability/llm/openai.rs`: route `OpenAiFunctionTool` construction through a shared private helper.
   - Scope: `src/capability/llm/openai.rs` only; allowed functions are `OpenAiFunctionTool::new(...)`, `OpenAiFunctionTool::with_description(...)`, and one new private helper such as `openai_function_tool(...)`. Do not change `OpenAiTool::function(...)`, `OpenAiFunctionCall`, `OpenAiToolCall`, `OpenAiChatRequest`, `tool_contract_valid(...)`, `push_tool_json(...)`, request JSON output, public struct fields, public signatures, retry-budget code, HTTP/client code, or NDJSON receipt code.
...
45. [ ] `SCORE_REPORT.md`: after items 43 and 44 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
