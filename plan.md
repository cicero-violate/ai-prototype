...
1. [x] `src/agent/cycle.rs`: consolidate `recovery_gate(...)` and `recovery_target_phase(...)` behind one private `RecoveryActionSpec` lookup while preserving both public-in-file helper names and all current action-to-gate/evidence/phase outputs.
