...
160. [ ] `src/bin/root_validate.rs`: extract a private `policy_reuse_common_regression_guards(...)` helper and add one unit test covering the first four policy-reuse retrieval-result regression compact modes.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --bin root_validate policy_reuse_common_regression_guards_preserve_initial_result_modes -- --test-threads=1`; then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin root_validate`.
