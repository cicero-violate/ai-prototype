...
Planning reconnaissance on 2026-05-12 confirms items 60 through 79 are complete with targeted and broader validation as recorded below. The next executable item is item 80, a graph-backed inspection of `src/agent/loop_driver.rs::LoopDriver::run_cycle(...)` using `SplitFn id=001e821dc83e940a`; the expected follow-up item is a helper extraction around planning-prompt context assembly only, preserving spawned/project turn selection, retry behavior, router calls, sleep cadence, and completion semantics. Item 78 inspected `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` and confirmed `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` with `expected_lo=3897`, `expected_hi=12284`, generated names `run__parse` and `run__transform`, and `delegate_strategy=preserve_original_signature`. Item 79 then extracted `run_recovery_phase(...)` with passing targeted cycle hash tests, `cargo fmt --check`, and full-suite validation. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
80. [ ] `src/agent/loop_driver.rs`: inspect graph-backed `SplitFn id=001e821dc83e940a` for `agent::loop_driver::LoopDriver::run_cycle` and write the smallest helper-extraction implementation checklist item for that exact function.
   ...
   - Validation: inspect `SplitFn id=001e821dc83e940a` (`fn_path=agent::loop_driver::LoopDriver::run_cycle`, `expected_lo=4201`, `expected_hi=7738`, generated names `run_cycle__parse` and `run_cycle__transform`, `delegate_strategy=preserve_original_signature`), inspect `src/agent/loop_driver.rs::LoopDriver::run_cycle(...)`, and run `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test agent::loop_driver::tests::project_prompts_do_not_claim_to_be_worker_certification -- --test-threads=1`.

81. [ ] `src/agent/loop_driver.rs`: extract planning-prompt context assembly from `LoopDriver::run_cycle(...)` into private helper `build_project_planning_prompt(goal: &str, agent_id: u32, agent_count: u32, working_dir: &Path) -> String` while preserving `LoopDriver::run_cycle(...) -> Result<(), String>` behavior.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test agent::loop_driver::tests::project_prompts_do_not_claim_to_be_worker_certification -- --test-threads=1 && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
