...
137. [ ] `src/capability/judgment/record.rs`: route `fold_ordered_policy_judgment_hash(...)`, `fold_ordered_policy_reuse_receipt_hash(...)`, and `fold_ordered_policy_reuse_content_hash(...)` through one private ordered `u64` hash-fold helper while preserving distinct caller domains.
