...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json`, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and source/test locations for the next current `ai` crate candidate in `src/api/server.rs` and `tests/api_server_contract.rs`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains a low-scoring graph row, but it is intentionally non-selectable. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis.

The selected graph-editor plan is schema version 1, contains 1,614 planned operations, and exposes one already-completed `SplitFn` candidate for `agent::loop_driver::LoopDriver::run_cycle` plus 1,613 `MergeFns` candidates. Previously selected local `ai` crate candidates through `agent::cycle`, `agent::objective`, and `agent::prompt` are complete with targeted and all-target validation evidence. Fresh selectable work starts with graph operation id `f7ae545a5fec7c8f`, a `MergeFns` candidate for `api::server::{evidence_from_str, gate_from_str}`. This is a small, file-local route-table consolidation that can improve Structure without touching `root_validate`, kernel state-machine semantics, transport ledgers, or capability policy. Generated canonical names are evidence, not mandatory names; implementation must preserve names consumed by existing call sites unless the selected item explicitly allows a call-site update. `root_validate` graph plans are non-selectable.

1. [x] `src/api/server.rs`: consolidate `gate_from_str(...)` and `evidence_from_str(...)` behind one private API submission token parser while preserving all accepted string literals, enum outputs, and invalid-payload errors.
   - Scope: `src/api/server.rs` functions `submission_from_dto(...)`, `gate_from_str(...)`, `evidence_from_str(...)`, and new private helper/type definitions immediately adjacent to those functions only.
...
4. [x] `SCORE_REPORT.md`: after items 1 through 3 land, refresh graph-derived evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
