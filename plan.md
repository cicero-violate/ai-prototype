...
46. [ ] `src/capability/llm/openai.rs`: route `OpenAiLlmEffectReceipt::{canonical_authority_hash, canonical_request_hash}` through one private ordered hash-fold helper while preserving distinct authority and request domains.
...
47. [ ] `src/lib.rs` test `openai_effect_receipt_hash_helpers_preserve_authority_request_domains`: add focused regression coverage for the shared effect-receipt hash helper.
