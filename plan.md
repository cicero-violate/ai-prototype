...
Planning reconnaissance on 2026-05-12 confirms items 60 through 96 are complete with targeted and broader validation as recorded below. The next executable item is item 97, source/graph inspection for `src/agent/loop_driver.rs::sync_mcp_workspace(...)` before a small TCP transport helper extraction is attempted in item 98 and focused helper tests are added in item 99. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the plan-introduced `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain candidate evidence only; the legitimate current source target is `src/agent/loop_driver.rs::sync_mcp_workspace(...)`, where the safest manual boundary is TCP request/response transport, not generated merge-surface edits. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
97. [ ] `src/agent/loop_driver.rs`: inspect `sync_mcp_workspace(mcp_url, project_dir)` and record the smallest safe helper-extraction boundary for transport/status handling.
   - Scope: `src/agent/loop_driver.rs`, `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json`, `plan.md`, and `status.md` only. Do not edit Rust source for this inspection item except planning/status evidence.
...
98. [ ] `src/agent/loop_driver.rs`: extract `sync_mcp_workspace(...)` TCP request/response transport into one private helper without changing endpoint parsing, HTTP request construction, status acceptance, or error semantics.
...
99. [ ] `src/agent/loop_driver.rs`: add focused unit tests for MCP workspace endpoint/status/request helpers.
