...
Planning reconnaissance on 2026-05-13 confirms items 60 through 119 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 120, a planning/inspection item for the graph-backed `examples/openai_tool_loop_trace.rs::submit_openai_tool_calls(...)` SplitFn candidate. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next work should continue small, graph-backed decomposition with validation evidence. Current auto-refactor evidence in `state/rustc/auto-refactor/..__state__rustc__openai_tool_loop_trace__bin__graph.graph-editor-plan.json` exposes `SplitFn id=9a947eeef45104ad` for `submit_openai_tool_calls` with fan-out `35`, rank `70`, expected range `4314..7017`, generated names `submit_openai_tool_calls__parse`/`submit_openai_tool_calls__transform`, and phases `parse`/`transform`; generated names remain evidence only. Source inspection confirms the likely safe extraction boundary is one private per-tool-call helper inside `examples/openai_tool_loop_trace.rs` that receives the already constructed `LiveSandboxProcessExecutor`, `OpenAiClient`, `process_receipt_path`, and `tool_call_index`, then performs the existing `tool_spec(...)`, LLM intent request/response validation, process execution, process receipt persistence, tool-result submission, and receipt return. `submit_openai_tool_calls(...)` should retain sandbox-root construction, executor configuration, receipt vector ownership, loop control, and final `submit_openai_process_receipt_batch(...)` call. Item 120 should record the exact boundary with compile evidence; item 121 should perform the helper extraction only after item 120 passes; item 122 refreshes graph-derived score evidence after item 121 lands.
...
120. [ ] `examples/openai_tool_loop_trace.rs`: inspect live graph-backed `submit_openai_tool_calls(...)` and record the exact helper boundary for the next execution item.
...
121. [ ] `examples/openai_tool_loop_trace.rs`: extract private helper `execute_one_openai_tool_call(...)` from `submit_openai_tool_calls(...)` for a single tool-call intent/execution/result cycle only.
...
122. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 121 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `status.md`, and `plan.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated or confirmed current, `status.md` records the new structural evidence, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy capability update.
   - Validation: `cargo run --manifest-path score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` plus `git diff -- SCORE_REPORT.md score.md status.md plan.md`.
