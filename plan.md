...
Planning reconnaissance on 2026-05-12 confirms items 60 through 68 are complete with targeted and broader validation as recorded below. The first incomplete executable item remains item 69, `src/agent/router.rs::build_streaming_http_request(...)`. Source reconnaissance found the helper and companion unit test already present in the working tree, but targeted validation failed because the test expected `Content-Length: 36` while the helper correctly emits `Content-Length: 34` for body `{"model":"gpt-test","stream":true}`. Current workspace reconnaissance also shows `src/agent/router.rs` is the only modified source file, so the next execution turn should finish item 69 by correcting that focused test assertion, applying rustfmt-compatible formatting in the same file, preserving the helper extraction, then running the named targeted, formatter, and full-suite validation. Python parsing of `state/rustc/auto-refactor/*.graph-editor-plan.json` reconfirmed graph-backed `SplitFn id=1ea38f3cc5f37f85` for `agent::router::send_streaming_request`; it remains smaller and lower-risk than `graph_mutation::generate_graph_patch`, `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process`, or connector/tool split candidates, and it directly improves call-structure evidence while Structure remains the lowest graph-derived axis in `SCORE_REPORT.md`.
...
69. [ ] `src/agent/router.rs`: finish the existing streaming HTTP request-string extraction by correcting `tests::streaming_http_request_builder_preserves_post_headers_and_body` to assert the helper-produced `Content-Length` for body `{"model":"gpt-test","stream":true}` while preserving private helper `build_streaming_http_request(path: &str, host: &str, port: u16, body: &str) -> String` and `send_streaming_request(config, body, logger, stream_deadline_ms) -> Result<crate::agent::sse::SseResult, OpenAiError>`.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test agent::router::tests::streaming_http_request_builder_preserves_post_headers_and_body -- --test-threads=1 && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
