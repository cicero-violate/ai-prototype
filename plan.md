...
Planning reconnaissance on 2026-05-12 confirms items 60 through 89 are complete with targeted and broader validation as recorded below. The next executable item is item 90, a graph-backed inspection of `examples/ollama_tool_mcp_loop_trace.rs::submit_llm_mcp_tool_calls(...)`. Current `SplitFn id=bf48e302ecddd798` for `submit_llm_mcp_tool_calls` is legitimate graph-backed evidence because the example still contains Ollama MCP call JSON construction, MCP tool-call validation, MCP execution, per-call receipt persistence, post-loop successful MCP receipt selection, `Command::SubmitEvidence`, `handle_envelope(...)`, and return-count finalization in one function. The proposed next helper boundary should mirror the already validated item 89 pattern as closely as the full MCP-JSON evidence model allows: post-loop combined MCP evidence submission only. Do not move Ollama request construction, JSON parsing/validation, MCP execution, per-call receipt persistence, tool-result mapping, or kernel/TLog authority into the helper. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
90. [ ] `examples/ollama_tool_mcp_loop_trace.rs`: inspect graph-backed `SplitFn id=bf48e302ecddd798` for `submit_llm_mcp_tool_calls` and record the smallest safe helper-extraction checklist item for the post-loop evidence-finalization boundary in that exact function.
