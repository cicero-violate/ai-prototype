...
119. [ ] `src/agent/cycle.rs`: add a private helper that centralizes submit-evidence hash wrapper delegation for `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, and `compute_envelope_hash(...)` without changing their public/private signatures.
