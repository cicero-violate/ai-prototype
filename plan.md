...
52. [ ] `src/capability/verification/proof.rs`: route `CanonicalEffect::{artifact, process, semantic_verification, policy, observation}` through one private named-kind constructor helper while preserving distinct effect kinds.
...
53. [ ] `src/lib.rs` test `canonical_effect_named_constructors_preserve_kind_and_validation_boundaries`: add focused regression coverage for the shared `CanonicalEffect` wrapper helper.
...
54. [ ] `SCORE_REPORT.md`: after items 52 and 53 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows including `ai` and any changed function counts, and `score.md` changes only if refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
