...
34. [ ] `src/capability/llm/openai.rs`: route `OpenAiJudgmentProofEvent::{expected_proof_hash, verifier_context_hash}` through one private ordered hash-fold helper while preserving distinct hash domains.
...
35. [ ] `src/lib.rs` test `openai_proof_event_hash_helpers_preserve_distinct_domains`: add focused regression coverage for the shared proof-event hash helper.
