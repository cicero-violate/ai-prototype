...
5. [ ] `src/agent/cycle.rs` test `recovery_route_table_preserves_failure_action_and_target_mappings`: update focused recovery-route regression coverage after item 4 so failure-to-action, action-to-gate, and action-to-target assertions all read through `recovery_action_spec(...)` rather than removed wrappers.
