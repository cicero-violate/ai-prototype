...
Planning reconnaissance on 2026-05-12 confirms items 60 through 94 are complete with targeted and broader validation as recorded below. The next executable item is item 95, source/graph inspection for `src/agent/loop_driver.rs::run_cycle(...)` before a small helper extraction is attempted in item 96. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the plan-introduced `LoopDriver::run_cycle` candidate remain candidate evidence only; the legitimate current source target is `src/agent/loop_driver.rs::run_cycle(...)`, where the safest manual boundary is prompt/label derivation for each turn, not generated merge-surface edits. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
95. [ ] `src/agent/loop_driver.rs`: inspect the current `LoopDriver::run_cycle(...)` body and record the smallest safe helper-extraction boundary for per-turn mode, label, and prompt derivation.
...
96. [ ] `src/agent/loop_driver.rs`: extract per-turn prompt/label derivation from `LoopDriver::run_cycle(...)` into a private helper while preserving `run_cycle(...) -> Result<(), String>` behavior.
