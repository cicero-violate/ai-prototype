...
116. [ ] `src/capability/judgment/record.rs`: route `policy_reuse_receipt_hash(...)` and `policy_reuse_trend_receipt_hash(...)` through existing private `fold_ordered_policy_reuse_receipt_hash(...)` while preserving distinct current-policy and trend receipt domains.
