...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json`, and the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten.

The selected graph-editor plan is schema version 1 and contains 1,614 planned operations: one already-completed `SplitFn` candidate for `agent::loop_driver::LoopDriver::run_cycle` plus 1,613 `MergeFns` candidates. The item-1 through item-3 `agent::config::{env_u32, env_u64}` candidate is complete. The next non-`root_validate` graph-backed surface is in `src/agent/cycle.rs`. Directly merging unrelated route/hash helpers is rejected as semantically unsafe, because private helper names document distinct contract, command, envelope, phase, evidence, effect, and recovery concepts even when their bodies are structurally similar. The safe executable subset is graph operation id `13d284226cb72df`, a `MergeFns` candidate for `agent::cycle::{recovery_gate, recovery_target_phase}`. Both are thin private wrappers over `recovery_action_spec(...)`, so the next work should use the shared `RecoveryActionSpec` directly while preserving recovery action, gate, target phase, sentinel, and submit-evidence behavior.

4. [x] `src/agent/cycle.rs`: remove private wrappers `recovery_gate(...)` and `recovery_target_phase(...)` by using `recovery_action_spec(...)` directly at recovery call sites.
   - Scope: `src/agent/cycle.rs` functions `run_recovery_phase(...)`, `recovery_gate(...)`, `recovery_target_phase(...)`, `recovery_action_spec(...)`, and tests that directly name the removed wrappers only.
   - Done when: `run_recovery_phase(...)` derives both `target_phase` and optional submit-evidence gate from the same `RecoveryActionSpec`; private wrappers `recovery_gate(...)` and `recovery_target_phase(...)` no longer exist; `recovery_action_spec(...)`, `recovery_action_for_failure(...)`, `recovery_route_for_action(...)`, and `recovery_route_for_failure(...)` keep their current semantics; no phase-route, hash-chain, worker, router, loop-driver, or TLog behavior changes.
...
5. [x] `src/agent/cycle.rs` test `recovery_route_table_preserves_failure_action_and_target_mappings`: update focused recovery-route regression coverage after item 4 so failure-to-action, action-to-gate, and action-to-target assertions all read through `recovery_action_spec(...)` rather than removed wrappers.
