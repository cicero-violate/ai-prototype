...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json`, `src/api/server.rs`, and `tests/api_server_contract.rs`. Items 152 through 163 are complete, reconciled, or historically blocked with recorded evidence; connector items 150 and 151 remain deferred until `../chatgpt-mcp-connector` source is mounted, and item 154 remains a historical local-root artifact-path blocker because `ai/state/rustc` remains empty. The user explicitly directed not to continue `root_validate` work. Current graph-derived evidence still keeps Structure as the lowest aggregate axis at `4.9`; `root_validate` remains the weakest Structure row at `1.5`, but it is intentionally not selectable. The next executable work is item 164: resolve the API server command-route contract blocker in `src/api/server.rs` against the three named `tests/api_server_contract.rs` failures that previously blocked broader validation. Item 165 is the follow-up all-target validation and graph-derived evidence review if item 164 changes source behavior. `score.md` project-level numeric scores remain unchanged because this planning turn selects validation-producing source work but does not itself add capability evidence.
...
Graph-derived score evidence in the local `SCORE_REPORT.md` now reports schema version 16, 17 crates, `G = 7.90 / 10`, Architecture `9.0`, Structure `4.9`, Simplicity `6.8`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`. The local `ai` library crate row reports 5,478 nodes, 34,134 edges, 2,214 functions, Architecture `9.3`, Structure `6.0`, Simplicity `7.5`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.6`; the local `chatgpt_mcp_connector` row reports 3,714 nodes, 21,661 edges, 1,704 functions, Architecture `9.0`, Structure `3.6`, Simplicity `7.4`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.3`; `root_validate` remains the weakest current Structure row at `1.5`. This refresh does not justify a project-level numeric score increase by itself. Item 148 completed the durable caller-selected evaluator helper-boundary inspection. Item 149 documented the source-availability blocker for `../chatgpt-mcp-connector/src/tools.rs` and `../chatgpt-mcp-connector/Cargo.toml`; the connector extraction remains deferred until the sibling source tree is exposed.
...
138. [x] `../chatgpt-mcp-connector/src/tools.rs`: inspect live graph-backed `execute_policy_selected_evaluator_suite_tool_inner(...)` and record the exact helper boundary for the next execution item.
   ...
   - Completed on 2026-05-13: inspection confirmed the graph-backed split candidate and recorded item 139's manual helper boundary. The helper should own policy authority rejection, intent parsing, suite and internal policy lookup preparation, scalar argument parsing, allowed-program parsing, and evaluator command policy construction only. The original function should retain workspace/cwd resolution, request construction, run/actor creation, tlog branching, and execution dispatch. Targeted validation passed with 6 policy-selected evaluator tests, 0 failures, and 528 filtered tests.

139. [x] `../chatgpt-mcp-connector/src/tools.rs`: extract one private helper from `execute_policy_selected_evaluator_suite_tool_inner(...)` for policy-selected evaluator transport input parsing and suite/policy lookup preparation only.
   ...
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_policy_selected_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: source inspection confirmed `execute_policy_selected_evaluator_suite_tool_inner(...)` delegates the item-139 boundary to private helper `prepare_policy_selected_evaluator_transport_inputs(...)`, while retaining workspace/cwd resolution, `CapabilityRequest` construction, run/actor creation, tlog-path branching, and execution dispatch. Targeted validation passed with 6 policy-selected evaluator tests, 0 failures, and 528 filtered tests; `cargo fmt --check` and `cargo check --manifest-path ../chatgpt-mcp-connector/Cargo.toml` also passed.

140. [x] `../chatgpt-mcp-connector/src/tools.rs`: add or tighten focused test coverage for the item-139 policy-selected evaluator transport input helper without changing evaluator execution semantics.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs` test module only. Add one focused regression test around `canon_execute_policy_selected_evaluator_suite` that proves the item-139 helper preserves default internal intent selection plus caller-provided scalar/allowed-program parsing through the public tool surface; acceptable assertions include stable response shape, `suite == "shell_smoke_validation"`, `suite_label == "shell smoke validation"`, caller `run_id`/`tlog_path` persistence when used, and durable evidence remaining in `Evaluating` without changing evaluator execution semantics.
   - Done when: the helper boundary introduced by item 139 has direct regression coverage beyond the existing policy-hit and policy-authority rejection tests, and the public MCP response shape remains unchanged.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_policy_selected_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: added `tools::tests::canon_execute_policy_selected_evaluator_suite_preserves_helper_input_parsing` in `../chatgpt-mcp-connector/src/tools.rs`. The test exercises the public `canon_execute_policy_selected_evaluator_suite` tool with default internal policy intent, caller `run_id`, caller `tlog_path`, explicit allowed programs `true` and `printf`, `timeout_ms`, and `max_output_bytes`, then verifies stable response shape, `shell_smoke_validation`, `shell smoke validation`, persisted durable TLog path, 4 entries, `Evaluating` status, policy lookup evidence, and evaluator output evidence. Targeted validation passed with 7 policy-selected evaluator tests, 0 failures, and 528 filtered tests after correcting the explicit allowlist to include both shell-smoke required programs. Broader `cargo test --all-targets` passed.

141. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after items 139-140 land, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
   - Completed on 2026-05-13: regenerated `SCORE_REPORT.md` from current `state/rustc` artifacts after item 140. The refreshed aggregate remained `G = 7.93 / 10` across 16 crates with 2 schema-version skips, Architecture `9.0`, Structure `4.8`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; `score.md` project-level numeric scores were intentionally unchanged because this was evidence refresh only.

142. [x] `../chatgpt-mcp-connector/src/tools.rs`: inspect live graph-backed `execute_evaluator_suite_tool_inner(...)` and record the exact helper boundary for the next execution item.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs::execute_evaluator_suite_tool_inner(...)`, adjacent helpers `parse_evaluator_suite_selection(...)`, `parse_allowed_programs(...)`, `EvaluatorCommandPolicy::with_output_limit(...)`, `execute_durable_selected_suite(...)`, `execute_in_memory_selected_suite(...)`, and `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`. Do not edit Rust source for this inspection item except planning/status evidence.
   - Done when: inspection confirms `SplitFn id=a57f325e92fb8f3f` still targets `tools::execute_evaluator_suite_tool_inner(...)` with expected range `21778..26627`, rejects generated names `execute_evaluator_suite_tool_inner__parse`/`execute_evaluator_suite_tool_inner__transform` as direct instructions, and records the item-143 manual boundary for one private helper that parses and resolves caller-selected evaluator transport inputs without changing public response semantics.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: inspected `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json` and confirmed `SplitFn id=a57f325e92fb8f3f` targets `tools::execute_evaluator_suite_tool_inner` with expected range `21778..26627`, split boundaries `phase::parse` and `phase::transform`, generated names `execute_evaluator_suite_tool_inner__parse` and `execute_evaluator_suite_tool_inner__transform`, and `preserve_original_signature`; generated names remain graph-editor evidence only. Source inspection found `execute_evaluator_suite_tool_inner(...)` at `../chatgpt-mcp-connector/src/tools.rs:555`, `execute_in_memory_selected_suite(...)` at line 683, `execute_durable_selected_suite(...)` at line 932, and `parse_allowed_programs(...)` at line 1597. The item-143 manual boundary is one private helper that extracts the current lines 559-584 and 605-622 responsibilities: required suite parsing/resolution, timeout/max-output/run/tlog/candidate/completion argument parsing, allowed-program parsing and fallback to `suite.required_programs()`, max-score derivation from `suite.commands`, and `EvaluatorCommandPolicy::with_output_limit(...)` construction. Keep workspace/cwd resolution, workspace-relative path calculation, `CapabilityRequest` construction, run/actor creation, tlog-path resolution, and durable/in-memory execution dispatch in `execute_evaluator_suite_tool_inner(...)`. Targeted validation passed with 16 evaluator-suite transport tests, 0 failures, and 519 filtered tests; broader `cargo test --all-targets` passed.

143. [x] `../chatgpt-mcp-connector/src/tools.rs`: reconcile and validate the existing private helper extraction from `execute_evaluator_suite_tool_inner(...)` for caller-selected evaluator transport input parsing and command-policy construction only.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs::execute_evaluator_suite_tool_inner(...)`, `../chatgpt-mcp-connector/src/tools.rs::CallerSelectedEvaluatorTransportInputs`, and `../chatgpt-mcp-connector/src/tools.rs::prepare_caller_selected_evaluator_transport_inputs(...)`. Do not change required `suite` validation, workspace/cwd resolution, `CapabilityRequest` construction, run id creation, actor id creation, durable-vs-in-memory branching, evaluator response shape, TLog persistence semantics, or public error semantics unless validation proves a defect.
   - Done when: source inspection confirms `execute_evaluator_suite_tool_inner(...)` still owns workspace/cwd resolution, request construction, run/actor creation, tlog-path branching, and execution dispatch, while delegating only suite parsing/resolution, timeout/max-output/run/tlog/candidate/completion argument parsing, allowed-program parsing, max-score derivation, and `EvaluatorCommandPolicy` construction to `prepare_caller_selected_evaluator_transport_inputs(...)`, and the targeted evaluator-suite transport tests pass.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: source inspection confirmed `CallerSelectedEvaluatorTransportInputs` and `prepare_caller_selected_evaluator_transport_inputs(...)` exist in `../chatgpt-mcp-connector/src/tools.rs`; `execute_evaluator_suite_tool_inner(...)` retains workspace/cwd resolution, workspace-relative path calculation, `CapabilityRequest` construction, run/actor creation, tlog-path resolution, and durable/in-memory dispatch while delegating only required suite parsing/resolution, scalar argument parsing, allowed-program parsing/fallback, max-score derivation, and `EvaluatorCommandPolicy::with_output_limit(...)` construction. Targeted validation passed with 16 evaluator-suite transport tests, 0 failures, and 519 filtered tests. The first raw validation attempt timed out before Rust output; the retry passed.

144. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 143 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
   - Completed on 2026-05-13: regenerated `SCORE_REPORT.md` after item 143. The graph-derived aggregate remains `G = 7.93 / 10` across 16 crates with 2 schema-version skips; axes remain Architecture `9.0`, Structure `4.8`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`. The refresh produced no project-level numeric score change because it confirmed existing structural evidence rather than proving a score-history-worthy capability change.

145. [x] `../chatgpt-mcp-connector/src/tools.rs`: inspect live graph-backed `execute_durable_policy_selected_suite(...)` and record the exact helper boundary for the next execution item.
   ...
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_policy_selected_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: inspection confirmed `SplitFn id=7f8fe031079d5622` still targets `tools::execute_durable_policy_selected_suite(...)` with expected range `40132..45772`, split boundaries `phase::parse`/`phase::transform`, generated names `execute_durable_policy_selected_suite__parse`/`execute_durable_policy_selected_suite__transform`, and `delegate_strategy=preserve_original_signature`; generated names remain evidence only. Source inspection found `execute_durable_policy_selected_suite(...)` at `../chatgpt-mcp-connector/src/tools.rs:1079`, `transport_accepted_evaluator_decision_with_supporting_evidence(...)` at line 1250, `transport_failed_evaluator_rejection_with_supporting_evidence(...)` at line 1318, and `evaluator_tool_response(...)` at line 1527. The item-146 manual boundary is one private helper for the post-policy-execution support and terminal-decision sequence only: derive `Vec<PolicySupportExecutionEvidence>` from passed/failed evaluator output, record policy-supported capability evidence on pass, create failed policy support evidence on failure, build optional acceptance summary when `complete_on_success` is true, start learning/complete accepted transport traces, or record candidate rejection for failed evaluators. Keep durable TLog creation, planning/execution transition order, policy-selected evaluator execution, partial-TLog error reporting, persisted verification, durable reopen, and `evaluator_tool_response(...)` construction in `execute_durable_policy_selected_suite(...)`. Targeted validation passed with 7 policy-selected evaluator tests, 0 failures, and 528 filtered tests; broader `cargo test --all-targets` passed.

146. [x] `../chatgpt-mcp-connector/src/tools.rs`: extract the private helper selected by item 145 from `execute_durable_policy_selected_suite(...)` without changing durable policy-selected evaluator semantics.
   ...
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_policy_selected_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: planning reconnaissance found the approved extraction already present as private helper `finalize_durable_policy_selected_evaluator_outcome(...)` in `../chatgpt-mcp-connector/src/tools.rs`. `execute_durable_policy_selected_suite(...)` delegates only the item-145 post-policy-execution support and terminal-decision sequence to that helper while keeping durable TLog creation/opening, planning/execution transition order, policy-selected evaluator execution, partial-TLog error reporting, persisted verification, durable reopen, and `evaluator_tool_response(...)` construction in the caller. Targeted validation passed with 7 policy-selected evaluator tests, 0 failures, and 528 filtered tests.

147. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 146 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
   - Completed on 2026-05-13: refreshed graph-derived structural report across 16 crates with 2 expected schema-version skips. Aggregate moved to `G = 8.02 / 10`, Architecture `8.9`, Structure `4.8`, Simplicity `7.4`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.5`; stderr captured score witness hash `6dad4f39c1c622a6aa8410ea1127f27278b381cd3a668e08f9c004514a036740`. `score.md` project-level numeric scores remain unchanged because this is graph-derived structural evidence refresh, not a new capability or validation boundary.

148. [x] `../chatgpt-mcp-connector/src/tools.rs`: inspect live graph-backed `execute_durable_selected_suite(...)` and record the exact helper boundary for the next execution item.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs::execute_durable_selected_suite(...)`, adjacent non-policy evaluator decision helpers `transport_accepted_evaluator_decision(...)`, `transport_failed_evaluator_rejection(...)`, durable orchestrator calls, and `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`. Do not edit Rust source for this inspection item except planning/status evidence.
   - Done when: inspection confirms `SplitFn id=4fcb62f370b7c63f` still targets `tools::execute_durable_selected_suite(...)` with expected range `35545..40130`, rejects generated names `execute_durable_selected_suite__parse`/`execute_durable_selected_suite__transform` as direct instructions, and records one manual helper boundary that reduces durable caller-selected evaluator orchestration complexity without changing public response semantics or persisted TLog semantics.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_evaluator_suite -- --test-threads=1`.
   - Completed on 2026-05-13: inspection confirmed `SplitFn id=4fcb62f370b7c63f` still targets `tools::execute_durable_selected_suite(...)` with expected range `35545..40130`, split boundaries `phase::parse`/`phase::transform`, generated names `execute_durable_selected_suite__parse`/`execute_durable_selected_suite__transform`, and `delegate_strategy=preserve_original_signature`; generated names remain evidence only. Source inspection found `execute_durable_selected_suite(...)` at `../chatgpt-mcp-connector/src/tools.rs:965`, `transport_accepted_evaluator_decision(...)` at line 1245, `transport_failed_evaluator_rejection(...)` at line 1314, and `evaluator_tool_response(...)` at line 1555. The item-149 manual boundary is one private helper for the post-evaluator terminal-decision sequence only: record capability/evaluation evidence on pass, build optional acceptance summary when `complete_on_success` is true, start learning/complete accepted transport traces, or record candidate rejection for failed evaluators. Keep durable TLog creation/opening, planning/execution transition order, evaluator execution, partial-TLog error reporting, persisted verification, durable reopen, and `evaluator_tool_response(...)` construction in `execute_durable_selected_suite(...)`. Targeted validation passed with 16 caller-selected evaluator tests, 0 failures, and 519 filtered tests.

149. [x] `../chatgpt-mcp-connector/src/tools.rs`: resolve or document the source-availability blocker for the durable caller-selected evaluator helper extraction.
   - Scope: workspace path evidence for `../chatgpt-mcp-connector/src/tools.rs`, `../chatgpt-mcp-connector/Cargo.toml`, and `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`; planning/status files only unless the missing sibling source tree is restored by the environment.
   - Done when: a shell check from the project root proves the sibling connector source and manifest are available for item 150, or `status.md` records the infrastructure/workspace blocker with exact path checks and item 150 remains blocked until the source tree is exposed.
   - Validation: `test -f ../chatgpt-mcp-connector/src/tools.rs && test -f ../chatgpt-mcp-connector/Cargo.toml`.
   - Completed on 2026-05-13 as a blocker record: validation returned `source_availability_rc=1`; `ls` reported `../chatgpt-mcp-connector`, `../chatgpt-mcp-connector/src/tools.rs`, and `../chatgpt-mcp-connector/Cargo.toml` do not exist from the project workspace. The extraction remains deferred until the sibling source tree is exposed.

Deferred connector work, not selectable until the sibling source tree is mounted:

150. `[blocked]` `../chatgpt-mcp-connector/src/tools.rs`: extract the private helper selected by item 148 from `execute_durable_selected_suite(...)` without changing durable caller-selected evaluator semantics.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs::execute_durable_selected_suite(...)` and one private helper in the same file selected by item 148. Do not change durable TLog creation/opening, planning/execution transition order, evaluator execution, partial-TLog error reporting, candidate rejection/acceptance semantics, completion behavior, or evaluator response shape.
   - Done when: `execute_durable_selected_suite(...)` delegates only the item-148 approved sub-sequence to a private helper, keeps durable orchestration and response construction auditable in the caller, and targeted caller-selected evaluator transport tests pass.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_evaluator_suite -- --test-threads=1`.

151. `[blocked]` `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 150 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.

152. [x] `src/bin/root_validate.rs`: inspect the compact-mode dispatch/catalog boundary and record the exact smallest follow-on test or helper boundary.
   - Scope: `src/bin/root_validate.rs::COMPACT_MODES`, `root_validate_dispatch_catalog_payload()`, `root_validate_dispatch_catalog_mode()`, `try_run_compact_mode(arg)`, `compact_mode_stdout_for_contract(arg, expected_marker)`, `main()`, and `state/rustc/root_validate__bin/graph.json`; planning/status files only for this inspection item.
   - Done when: inspection records whether the next executable work should be one focused unit test or one helper extraction, names the exact function/test boundary, and confirms no generated merge-surface recommendation is being applied blindly.
   - Validation: `cargo check --bin root_validate`.
   - Completed on 2026-05-13: source inspection found `COMPACT_MODES`, `root_validate_dispatch_catalog_payload()`, `root_validate_dispatch_catalog_mode()`, `try_run_compact_mode(arg)`, `compact_mode_stdout_for_contract(arg, expected_marker)`, `main()`, and `mirror_validation_outcome(...)` in `src/bin/root_validate.rs`. The mounted workspace exposes no `state/rustc/root_validate__bin/graph.json` or auto-refactor plan files, so no generated graph-editor operation was used. The smallest follow-on boundary is one unit test named `root_validate_dispatch_catalog_lists_every_compact_mode_once` in `src/bin/root_validate.rs` that verifies the dispatch-catalog payload covers every `COMPACT_MODES` entry exactly once, includes `--root-validate-dispatch-catalog=>canon_root_validate_dispatch_catalog_v1`, and that `try_run_compact_mode("--root-validate-dispatch-catalog")` emits the schema, record type, compact-mode count, and hash fields without invoking full root validation. Validation passed with `cargo check --bin root_validate`.

153. [x] `src/bin/root_validate.rs`: add unit test `root_validate_dispatch_catalog_lists_every_compact_mode_once`.
   - Scope: one focused `#[cfg(test)]` unit test in `src/bin/root_validate.rs` covering `root_validate_dispatch_catalog_payload()`, `COMPACT_MODES`, and `try_run_compact_mode("--root-validate-dispatch-catalog")`. Do not change compact mode names, markers, exit codes, `mirror_validation_outcome(...)`, full root validation fallback behavior, or JSON record schemas.
   - Done when: the named test proves the dispatch catalog covers every compact mode exactly once, includes the dispatch-catalog mode itself, and the compact-mode JSON contains schema `canon_root_validate_dispatch_catalog_v1`, record type `root_validate_dispatch_catalog`, compact-mode count, and `dispatch_catalog_hash`.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --bin root_validate root_validate_dispatch_catalog_lists_every_compact_mode_once -- --test-threads=1` passed on 2026-05-13 with 1 test and 0 failures. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed on 2026-05-13, including the new `root_validate_bin::tests::root_validate_dispatch_catalog_lists_every_compact_mode_once` integration-visible run.
   - Completed on 2026-05-13: `src/bin/root_validate.rs` now contains `root_validate_dispatch_catalog_lists_every_compact_mode_once`, which verifies dispatch-catalog uniqueness, full `COMPACT_MODES` coverage, the self-catalog entry `--root-validate-dispatch-catalog=>canon_root_validate_dispatch_catalog_v1`, successful `try_run_compact_mode("--root-validate-dispatch-catalog")`, and compact-mode JSON fields for schema, record type, compact-mode count, and `dispatch_catalog_hash`.

154. `[blocked]` `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 153 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
...
156. [x] `SCORE_REPORT.md`: rerun the graph-derived structural evidence refresh after item 155 produces compatible graph artifacts, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` remains blocked against the empty local `ai/state/rustc`; item 155's verified compatible graph root is `../state/rustc`, and `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` passed on 2026-05-13.
   - Completed on 2026-05-13: item 155 `bash scripts/recapture_rustc_graphs.sh --check` passed first, proving compatible graph artifacts for required `ai/graph.json` and `root_validate__bin/graph.json` under the configured artifact root. The score refresh then regenerated `SCORE_REPORT.md` from `../state/rustc` across 20 crates with 0 skipped: aggregate `G = 7.97 / 10`, Architecture `8.9`, Structure `4.8`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`. `score.md` rationale was updated to the refreshed graph-derived values only; project-level numeric capability scores remain unchanged because this was evidence refresh, not a new capability boundary.


157. [x] `src/bin/agent.rs`: inspect graph-backed `supervisor_reload_worker_port(...)` and record the exact smallest helper boundary for the next execution item.
   - Scope: `src/bin/agent.rs::supervisor_reload_worker_port(...)`, `parse_reload_worker_port(response)`, `resolve_single_cycle_worker()`, and `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__agent__bin__graph.graph-editor-plan.json`; planning/status files only for this inspection item.
   - Done when: inspection confirms `SplitFn id=f6def77b91796a65` still targets `supervisor_reload_worker_port(...)` with expected range `3863..5175`, rejects generated names `supervisor_reload_worker_port__parse`/`supervisor_reload_worker_port__transform` as direct instructions, and records one manual helper boundary that reduces HTTP request/response orchestration complexity without changing fallback behavior, timeout behavior, or `parse_reload_worker_port(...)` semantics.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin agent` passed on 2026-05-13.
   - Completed on 2026-05-13: inspection confirmed `SplitFn id=f6def77b91796a65` still targets `src/bin/agent.rs::supervisor_reload_worker_port(...)` with expected range `3863..5175`, split boundaries `phase::parse`/`phase::transform`, generated names `supervisor_reload_worker_port__parse`/`supervisor_reload_worker_port__transform`, and `delegate_strategy=preserve_original_signature`; generated names remain evidence only. Source inspection found `resolve_single_cycle_worker()` at line 78, `supervisor_reload_worker_port(...)` at line 108, and `parse_reload_worker_port(response)` at line 146. The item-158 manual helper boundary is one private helper for only the HTTP exchange sequence: connect to `127.0.0.1:supervisor_port`, set read/write timeouts, send the `POST /reload` request, flush, read the response string, and return the raw response. Keep fallback status routing for HTTP `200`/`204`/other status, status-line parsing, error messages, timeout durations, and `parse_reload_worker_port(response)` in the existing caller.

158. [x] `src/bin/agent.rs`: extract the private helper selected by item 157 from `supervisor_reload_worker_port(...)` without changing supervisor reload semantics.
   - Scope: `src/bin/agent.rs::supervisor_reload_worker_port(...)` and one private helper in the same file selected by item 157. Do not change fallback behavior for HTTP `204`, status routing, timeout durations, error message prefixes, request path, `parse_reload_worker_port(response)`, `resolve_single_cycle_worker()`, or public single-cycle behavior.
   - Done when: `supervisor_reload_worker_port(...)` delegates only the item-157 approved HTTP exchange sequence to a private helper and keeps status-line parsing plus fallback routing auditable in the caller.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin agent` passed on 2026-05-14.
   - Completed on 2026-05-14: `src/bin/agent.rs` now contains private helper `supervisor_reload_http_exchange(supervisor_port) -> Result<String, String>` for only the item-157 HTTP exchange sequence. `supervisor_reload_worker_port(...)` still owns status-line parsing, HTTP `200`/`204`/other routing, fallback behavior, and `parse_reload_worker_port(response)` invocation. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed.

159. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 158 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` passed on 2026-05-14.
   - Completed on 2026-05-14: graph artifact check passed for the configured `../state/rustc` root, `SCORE_REPORT.md` regenerated across 17 crates with 0 skipped, and `score.md` rationale was refreshed to `G = 7.98 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4` without changing project-level numeric scores. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed.

160. [x] `src/bin/root_validate.rs`: extract a private `policy_reuse_common_regression_guards(...)` helper and add one unit test covering the first four policy-reuse retrieval-result regression compact modes.
   - Scope: `src/bin/root_validate.rs` only; allowed functions/tests are `policy_reuse_evidence_retrieval_result_manifest_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_admission_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_manifest_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_readiness_regression_smoke_mode()`, new private helper `policy_reuse_common_regression_guards(...)`, and new unit test `policy_reuse_common_regression_guards_preserve_initial_result_modes`.
   - Done when: the four named regression smoke functions delegate the shared `is_valid`, no retrieval read/write/query execution, no runtime-result approval, no policy promotion, no student training, external evidence present, and `!passed()` checks to `policy_reuse_common_regression_guards(...)`; each function keeps its stage-specific predecessor/status/reason assertions local; the new test invokes the four exact compact-mode args `--policy-reuse-evidence-retrieval-result-manifest-regression-smoke`, `--policy-reuse-evidence-retrieval-result-use-admission-regression-smoke`, `--policy-reuse-evidence-retrieval-result-use-manifest-regression-smoke`, and `--policy-reuse-evidence-retrieval-result-use-readiness-regression-smoke` through `try_run_compact_mode(...)` and asserts exit code `0` plus the expected not-ready/not-admitted status strings.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --bin root_validate policy_reuse_common_regression_guards_preserve_initial_result_modes -- --test-threads=1`; then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin root_validate` passed on 2026-05-14.
   - Completed on 2026-05-14: `src/bin/root_validate.rs` now contains private helper `policy_reuse_common_regression_guards(...)`. The four named initial policy-reuse retrieval-result regression smoke functions delegate only the shared validity/no-side-effect/external-evidence/not-passed checks to that helper while retaining their predecessor/status/reason assertions locally. New unit test `policy_reuse_common_regression_guards_preserve_initial_result_modes` invokes the four exact compact-mode args through `try_run_compact_mode(...)` and verifies exit code `0` plus the expected not-ready/not-admitted status strings. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed.

161. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 160 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows with special attention to the `root_validate` Structure row, and `score.md` changes only if the refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` passed on 2026-05-14.
   - Completed on 2026-05-14: graph artifact check passed for the configured `../state/rustc` root, `SCORE_REPORT.md` regenerated across 17 crates with 0 skipped, and `score.md` rationale was refreshed to `G = 7.90 / 10`, Architecture `9.0`, Structure `4.9`, Simplicity `6.8`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2` without changing project-level numeric scores. `root_validate` Structure remained `1.5`; Architecture moved from `3.2` to `3.3`. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed.

162. [x] `src/bin/root_validate.rs`: extend `policy_reuse_common_regression_guards(...)` to the next four adjacent policy-reuse retrieval-result regression compact modes and add one unit test covering those four modes.
   - Scope: `src/bin/root_validate.rs` only; allowed functions/tests are `policy_reuse_evidence_retrieval_result_use_approval_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_manifest_admission_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_summary_regression_smoke_mode()`, `policy_reuse_evidence_retrieval_result_use_summary_manifest_regression_smoke_mode()`, existing private helper `policy_reuse_common_regression_guards(...)`, and new unit test `policy_reuse_common_regression_guards_preserve_follow_on_result_modes`.
   - Done when: the four named follow-on regression smoke functions delegate only the shared `is_valid`, no retrieval read/write/query execution, no runtime-result approval, no policy promotion, no student training, external evidence present, and `!passed()` checks to `policy_reuse_common_regression_guards(...)`; each function keeps its stage-specific predecessor/status/reason assertions local; the new test invokes the four exact compact-mode args `--policy-reuse-evidence-retrieval-result-use-approval-regression-smoke`, `--policy-reuse-evidence-retrieval-result-use-manifest-admission-regression-smoke`, `--policy-reuse-evidence-retrieval-result-use-summary-regression-smoke`, and `--policy-reuse-evidence-retrieval-result-use-summary-manifest-regression-smoke` through `try_run_compact_mode(...)` and asserts exit code `0` plus the expected not-ready/not-admitted/not-approved status strings.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --bin root_validate policy_reuse_common_regression_guards_preserve_follow_on_result_modes -- --test-threads=1`; then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin root_validate`.
   - 2026-05-14 implementation note: the source change exists in the working tree and both named item validations passed, but the item is intentionally left unchecked and uncommitted because the required broader gate `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` failed in out-of-scope `tests/api_server_contract.rs` cases against pre-existing `src/api/server.rs` changes. The failing cases are `command_route_maps_tlog_persistence_failure_to_internal_server_error`, `command_route_uses_transport_session_and_persists_tlog`, and `command_route_replays_after_durable_resume_without_appending_tlog`.
   - 2026-05-14 implementation step 2 note: reran the two named item validations successfully, then reran the required broader gate. `cargo test --all-targets` still fails only in the same out-of-scope `tests/api_server_contract.rs` cases above, so this item remains unchecked and uncommitted until that blocker is resolved or isolated.
   - 2026-05-14 implementation step 3 note: reran the two named item validations successfully again. The required broader gate still fails in the same three out-of-scope `tests/api_server_contract.rs` cases above, so this item remains unchecked and uncommitted.
   - 2026-05-14 implementation step 4 note: reran the two named item validations successfully again. The required broader gate still fails in the same three out-of-scope `tests/api_server_contract.rs` cases above, so this item remains unchecked and uncommitted.
   - 2026-05-14 implementation step 5 note: reran the two named item validations successfully again. The required broader gate still fails in the same three out-of-scope `tests/api_server_contract.rs` cases above, so this item remains unchecked and uncommitted.
   - Completed on 2026-05-14 implementation step 3: restored the scoped `src/bin/root_validate.rs` file from `HEAD`, extended `policy_reuse_common_regression_guards(...)` delegation to the four follow-on retrieval-result regression smoke modes, and added unit test `policy_reuse_common_regression_guards_preserve_follow_on_result_modes`. Targeted validation passed with `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --bin root_validate policy_reuse_common_regression_guards_preserve_follow_on_result_modes -- --test-threads=1` and `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin root_validate`. Broader validation passed with `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.

163. [x] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 162 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows with special attention to the `root_validate` Structure row, and `score.md` changes only if the refreshed evidence differs from the current rationale.
   ...
   - Completed on 2026-05-14 implementation step 4: graph artifact check passed for the configured `../state/rustc` root, `SCORE_REPORT.md` regenerated across 17 crates with 0 skipped, and `score.md` was reviewed without numeric-score changes because the refreshed evidence remains a structural report refresh rather than a score-history-worthy capability boundary. Refreshed aggregate remains `G = 7.90 / 10`, Architecture `9.0`, Structure `4.9`, Simplicity `6.8`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; `root_validate` now reports 163 nodes, 1,990 edges, 161 functions, Architecture `3.3`, Structure `1.5`, Simplicity `6.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `7.3`. Broader validation `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets` passed.


164. [ ] `src/api/server.rs`: restore `/v1/command` acceptance, replay, and persistence-error behavior expected by the API server contract tests.
   - Scope: `src/api/server.rs::post_command(...)`, `decode_command(...)`, and directly supporting private helpers in `src/api/server.rs` only; the targeted tests are `tests/api_server_contract.rs::command_route_uses_transport_session_and_persists_tlog`, `tests/api_server_contract.rs::command_route_maps_tlog_persistence_failure_to_internal_server_error`, and `tests/api_server_contract.rs::command_route_replays_after_durable_resume_without_appending_tlog`. Do not edit `src/bin/root_validate.rs`, compact-mode tests, graph score tooling, or unrelated API routes.
   - Done when: the three named API server contract tests pass, accepted command submissions return HTTP `200` with disposition `accepted`, replayed durable submissions return HTTP `200` with disposition `replayed` without appending to the TLog, and persistence failure after an otherwise valid command maps to HTTP `500` with `TlogIo`.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --test api_server_contract command_route -- --test-threads=1`.

165. [ ] `cargo test --all-targets`: rerun the broader validation gate after item 164, then refresh graph-derived evidence only if source behavior changed.
   - Scope: validation and evidence files only: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md`. Do not edit `src/bin/root_validate.rs`.
   - Done when: `cargo test --all-targets` passes, `status.md` records the full-suite result, and `score.md` changes only if the evidence justifies a score-history-worthy scoring change.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
