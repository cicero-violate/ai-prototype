...
Planning reconnaissance on 2026-05-12 confirms items 60 through 85 are complete with targeted and broader validation as recorded below. The next executable item is item 86, a graph-backed inspection of `examples/openai_tool_loop_trace.rs::submit_openai_tool_calls(...)`. Item 85 extracted `run_llm_gate_phase(...)` from `src/agent/cycle.rs::AgentCycle::run(...)` with passing cycle hash validation, `cargo fmt --check`, and full-suite validation. Current `SplitFn id=9a947eeef45104ad` for `submit_openai_tool_calls` is legitimate graph-backed evidence because the example still contains a long tool-call loop plus post-loop batch hash, `Command::SubmitProcessReceiptBatch`, `handle_envelope(...)`, and return-count finalization in one function. The proposed next helper boundary is post-loop process-receipt batch submission only; do not move OpenAI request construction, LLM intent validation, sandbox execution, per-call receipt persistence, tool-result submission, or kernel/TLog authority into the helper. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
86. [ ] `examples/openai_tool_loop_trace.rs`: inspect graph-backed `SplitFn id=9a947eeef45104ad` for `submit_openai_tool_calls` and record the smallest safe helper-extraction checklist item for the post-loop batch receipt submission boundary in that exact function.
