...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json`, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and source locations for current `ai` crate candidates in `src/agent/objective.rs`, `src/agent/prompt.rs`, and `src/agent/cycle.rs`. User direction is explicit: do not continue `root_validate` work. `root_validate` remains a low-scoring graph row, but it is intentionally non-selectable. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. The next executable work is item 6, a focused `src/agent/objective.rs` regression test that proves the just-landed graph-backed hash-setter merge preserved independent field behavior.

The selected graph-editor plan is schema version 1, contains 1,614 planned operations, and exposes one already-completed `SplitFn` candidate for `agent::loop_driver::LoopDriver::run_cycle` plus 1,613 `MergeFns` candidates. Previously selected candidates for `agent::config::{env_u32, env_u64}`, `agent::router::{env_u32, env_u64}`, `agent::cycle::{compute_envelope_hash, compute_evidence_contract_hash, compute_submit_evidence_command_hash}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `agent::cycle::{gate_id_u64, evidence_u64_value, effect_for_gate_evidence}`, `agent::cycle::{use_teacher, phase_gate}`, `agent::cycle::{recovery_action_for_failure, recovery_action_spec}`, and `agent::objective::{AgentObjective::with_risk_envelope, AgentObjective::with_stop_condition}` are complete. Fresh selectable work starts with the remaining local `ai` crate regression and merge-surface helpers in `src/agent/objective.rs` and `src/agent/prompt.rs`, covering graph operation id `aba6f10216943c56` with regression evidence and prompt merge operation ids `aa3a4dfa6fcc7d62`, `989423f5fe460289`, `5f55ce19806b52b6`, `69419bb91afee3e5`, `5e56172e32b32d5a`, `d66440ab1ee7652e`, `46e75fc66d84c9c8`, `1fbbcfa859088f35`, `d0e66301030c7809`, and `df5857ef82c36720`. Generated canonical names are evidence, not mandatory names; implementation must preserve names consumed by existing call sites unless the selected item explicitly allows a call-site update. `root_validate` graph plans are non-selectable.

1. [x] `src/agent/cycle.rs`: consolidate `use_teacher(...)` and `phase_gate(...)` behind one private phase-route lookup while preserving all current phase-to-teacher and phase-to-gate/evidence outputs.
   - Scope: `src/agent/cycle.rs` functions `use_teacher(...)`, `phase_gate(...)`, and a new private helper/type immediately adjacent to the phase dispatch helpers only.
...
5. [x] `src/agent/objective.rs`: consolidate `with_risk_envelope(...)` and `with_stop_condition(...)` through one private objective-hash setter while preserving `AgentObjective` fields, hash values, builder return semantics, and call-site names.
   - Scope: `src/agent/objective.rs` functions `AgentObjective::with_risk_envelope(...)`, `AgentObjective::with_stop_condition(...)`, and a new private helper enum/function in the same file only.
