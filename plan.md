...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 110 through 112 are complete and that no unchecked executable item remains before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; rejected already reconciled higher-ranked non-`root_validate` candidates in `agent::cycle`, `agent::loop_driver`, `agent::prompt`, `agent::worker_client`, `api::transport`, `capability::eval::record`, and policy-judgment helpers; inspected candidate source and tests in `src/capability/judgment/record.rs`; and checked the current graph-derived structural evidence in `SCORE_REPORT.md`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning selected graph operations `2113f28c435b03f0` and `e6171d384ff70781`, covering `capability::judgment::record::{policy_reuse_cost_catalog_content_hash, policy_reuse_evaluator_savings_content_hash}`, as the next safe non-`root_validate` consolidation area. The safe subset is private ordered policy-reuse content hash folding only: keep both content-hash function names and signatures, preserve each distinct domain seed, preserve all source-hash and version validity guards, preserve record-type and status/reason code mappings, preserve field order, preserve non-zero `h.max(1)` behavior, preserve catalog and savings receipt wrappers, preserve existing policy-reuse performance-cost trend behavior, and leave `root_validate` unchanged.

113. [x] `src/capability/judgment/record.rs`: route `policy_reuse_cost_catalog_content_hash(...)` and `policy_reuse_evaluator_savings_content_hash(...)` through one private ordered policy-reuse content hash-fold helper while preserving distinct catalog and evaluator-savings domains.
   - Scope: `src/capability/judgment/record.rs` only; allowed production functions are `policy_reuse_cost_catalog_content_hash(...)`, `policy_reuse_evaluator_savings_content_hash(...)`, and at most one new private helper adjacent to the policy-reuse content hash helpers. Do not change `PolicyReuseCostCatalogReceipt`, `PolicyReuseEvaluatorSavingsReceipt`, `PolicyReusePerformanceCostTrendReceipt`, `policy_reuse_cost_catalog_receipt_hash(...)`, `policy_reuse_evaluator_savings_receipt_hash(...)`, `policy_reuse_performance_cost_trend_receipt_hash(...)`, policy judgment records, JSON encoders, constructors, validation methods, API code, LLM code, or `root_validate` in this item.
   - Done when: `policy_reuse_cost_catalog_content_hash(...)` still rejects zero schema/catalog/source hashes and invalid record-type or missing-required-modes codes; still uses seed `0x504f_4c52_4341_5441u64`; still folds schema version, record type code, catalog version, evidence-family count, healthy-mode count, regression-mode count, retained-fixture count, required-mode booleans, summary-complete flag, missing-required-modes code, and all six source hashes in the existing order; `policy_reuse_evaluator_savings_content_hash(...)` still rejects zero schema/savings/source hashes and invalid record-type or regression-reason codes; still uses seed `0x504f_4c52_4556_5341u64`; still folds schema version, record type code, savings version, three source hashes, sample runs, policy hits, avoided calls, estimated saved units, baseline calls, actual calls, reduction ratio, validation flag, and regression-reason code in the existing order; both functions delegate through the same private ordered fold helper; and no public API, receipt, replay, JSON, policy, filesystem, process, or network behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

114. [x] `src/capability/judgment/record.rs` test `policy_reuse_content_hash_helpers_preserve_catalog_and_savings_boundaries`: add focused regression coverage for the shared policy-reuse content hash helper.
   - Scope: `src/capability/judgment/record.rs` test module only; use existing `PolicyReuseCostCatalogReceipt`, `PolicyReuseEvaluatorSavingsReceipt`, `PolicyReuseReceipt`, `PolicyReuseScaleTraceReceipt`, and `PolicyReusePerformanceCostTrendReceipt` constructors and private test access to the content and receipt hash helpers. Do not change production code in this item.
   - Done when: the named test proves catalog and evaluator-savings receipts remain valid; their content hashes are non-zero and distinct for related policy-reuse evidence; catalog source-hash, missing-required-modes, and summary-complete tampering are rejected; evaluator-savings source-hash, regression-reason, validation flag, and savings-metric tampering are bound to the savings content hash; wrapper receipt hashes still require the embedded content hash fields; and the test performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test policy_reuse_content_hash_helpers_preserve_catalog_and_savings_boundaries -- --test-threads=1`.

115. [x] `SCORE_REPORT.md`: after items 113 and 114 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 107 through 109 are complete and that no unchecked executable item remains before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; inspected candidate source and tests in `src/capability/llm/record.rs` and `src/lib.rs`; and checked the working tree through the project connector. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning selected graph operation `95dfbd77e8746d4f`, covering `capability::llm::record::{llm_payload_hash, prompt_hash}`, as the next safe non-`root_validate` consolidation area. The safe subset is private ordered hash folding only: keep all semantic hash function names and call sites, preserve each seed, preserve field order, preserve `h.max(1)` behavior, preserve validity and submission semantics, preserve `LlmRecord::from_context_policy(...)` and `LlmRecord::from_external_response(...)`, and leave `root_validate` unchanged.

110. [x] `src/capability/llm/record.rs`: route `prompt_hash(...)`, `response_hash(...)`, `rationale_hash(...)`, and `llm_payload_hash(...)` through one private ordered hash-folding helper while preserving each semantic hash domain.
   - Scope: `src/capability/llm/record.rs` only; allowed production functions are `prompt_hash(...)`, `response_hash(...)`, `rationale_hash(...)`, `llm_payload_hash(...)`, and at most one new private helper adjacent to those functions. Do not change `LlmRecord`, `LlmPromptRecord`, `LlmResponseRecord`, `LlmStructuredAdapter`, retry-budget helpers, `token_count(...)`, public APIs, call sites outside this file, tests, or `root_validate` in this item.
   - Done when: every existing hash function keeps its own seed and public/private function name; every mixed field remains in the same order as before; `h.max(1)` non-zero behavior is preserved for all four hash functions; `LlmRecord::submission(...)` still derives payload evidence from `llm_payload_hash(self)`; and no validation, judgment, policy, retry-budget, filesystem, process, network-target, or runtime behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

111. [x] `src/lib.rs` test `llm_record_hash_fold_helper_preserves_hash_domains`: add focused regression coverage for the shared LLM record hash-folding helper.
   - Scope: `src/lib.rs` test module only; construct deterministic `ContextRecord`, `PolicyStore`, and `LlmRecord` values through existing public or crate-visible APIs. Do not change production code in this item.
   - Done when: the named test proves prompt, response, rationale, and payload hashes remain non-zero; proves prompt, response, rationale, and payload hash domains remain distinct for a valid record; proves changing an external response hash changes response/rationale/payload evidence without changing the prompt hash; proves `submission().payload_hash` tracks the record payload hash boundary; and performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test llm_record_hash_fold_helper_preserves_hash_domains -- --test-threads=1`.

112. [x] `SCORE_REPORT.md`: after items 110 and 111 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows including `ai` and any changed function counts, and `score.md` changes only if refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.

Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 104 through 106 are complete and that implementation steps 2 through 5 found no selectable unchecked item after item 106; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; rejected already reconciled higher-ranked candidates in `agent::cycle`, `agent::loop_driver`, `agent::prompt`, `agent::objective`, `api::transport`, and OpenAI/Ollama constructor or encoder surfaces; inspected candidate source and tests in `src/agent/worker_client.rs` and nearby `src/agent/cycle.rs` worker-client call sites; and checked the working tree through the project connector. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `9.0`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning selected graph operation `6e26c91f6053b823`, covering `agent::worker_client::WorkerClient::{new, new_with_timeout}`, as the next safe non-`root_validate` consolidation area. The safe subset is private constructor delegation only: keep both public constructor names and signatures, preserve `DEFAULT_TIMEOUT_MS`, preserve custom timeout conversion through `Duration::from_millis(...)`, preserve `from_env()` default-constructor behavior, preserve port storage, preserve health/state/submit request behavior, preserve TCP timeout semantics, and leave `root_validate` unchanged.

107. [x] `src/agent/worker_client.rs`: route `WorkerClient::new(...)` and `WorkerClient::new_with_timeout(...)` through one private constructor helper while preserving default and custom timeout semantics.
   - Scope: `src/agent/worker_client.rs` only; allowed production functions are `WorkerClient::new(...)`, `WorkerClient::new_with_timeout(...)`, `WorkerClient::from_timeout_ms(...)`, and at most one new private helper adjacent to the constructors. Do not change `DEFAULT_TIMEOUT_MS`, `WorkerClientError`, `WorkerResponse`, `from_env(...)`, `health(...)`, `state(...)`, `submit_command(...)`, `get(...)`, `post(...)`, `send(...)`, `parse_response(...)`, TCP request formatting, tests, or `root_validate` in this item.
   - Done when: `WorkerClient::new(port)` still stores the supplied port and `Duration::from_millis(DEFAULT_TIMEOUT_MS)`; `WorkerClient::new_with_timeout(port, timeout_ms)` still stores the supplied port and `Duration::from_millis(timeout_ms)`; `from_env()` still constructs through the default constructor; both public constructors delegate through the same private constructor boundary; and no public API, HTTP wire format, worker route, TCP timeout, filesystem, process, network-target, or runtime behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

108. [x] `src/agent/worker_client.rs` test `worker_client_constructor_helper_preserves_timeout_boundaries`: add focused regression coverage for the shared worker-client constructor helper.
   - Scope: `src/agent/worker_client.rs` test module only; use existing private module access to inspect `WorkerClient` fields and `DEFAULT_TIMEOUT_MS`. Do not change production code in this item.
...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 101 through 103 are complete and that there was no first incomplete executable item before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; rejected stale/no-op candidates already backed by helpers or tests, including `agent::cycle::recovery_route_for_action`/`recovery_route_for_failure`; inspected candidate source and tests in `src/api/transport.rs` and `tests/api_transport_contract.rs`; and checked the working tree through the project connector. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning selected graph operation `8cbb45346f344059`, covering `api::transport::ApiTransportLedger::{receipt_for, receipt_for_request_id}`, as the next safe non-`root_validate` consolidation area. The safe subset is private receipt lookup predicate delegation only: keep both existing lookup names and semantics, preserve exact frame match behavior via `ApiTransportFrame::matches_receipt(...)`, preserve request-id-only membership lookup, preserve payload-scoped conflict detection, preserve duplicate request-id rejection, preserve persisted receipt loading, preserve TLog replay verification, and leave `root_validate` unchanged.

104. [x] `src/api/transport.rs`: route `ApiTransportLedger::receipt_for(...)` and `ApiTransportLedger::receipt_for_request_id(...)` through one private receipt lookup helper while preserving frame-match and request-id lookup semantics.
   - Scope: `src/api/transport.rs` only; allowed production functions are `ApiTransportLedger::receipt_for(...)`, `ApiTransportLedger::receipt_for_request_id(...)`, `ApiTransportLedger::receipt_for_frame_request_id(...)`, `ApiTransportLedger::contains_request_id(...)`, `ApiTransportLedger::has_conflicting_request(...)`, and at most one new private helper inside `impl ApiTransportLedger`. Do not change `ApiTransportReceipt`, `ApiTransportFrame`, `push_receipt(...)`, `push_response(...)`, `handle_transport_frame_once(...)`, NDJSON load/encode functions, TLog verification functions, session code, tests, or `root_validate` in this item.
   - Done when: `receipt_for(frame)` still returns only receipts accepted by `frame.matches_receipt(...)`; `receipt_for_request_id(request_id)` still returns any receipt with the same `request_id` regardless of payload or command hash; `contains_request_id(...)` and `has_conflicting_request(...)` retain their exact behavior; both lookup wrappers delegate through the same private helper boundary; and no public API, persistence format, replay verification, state transition, TLog, filesystem, process, or network behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

105. [x] `src/api/transport.rs` test `api_transport_ledger_lookup_helper_preserves_frame_and_request_boundaries`: add focused regression coverage for the shared transport-ledger receipt lookup helper.
   - Scope: `src/api/transport.rs` test module only; use existing private module access and construct `ApiTransportFrame`, `ApiTransportReceipt`, and `ApiTransportLedger` values directly. Do not change production code in this item.
   - Done when: the named test proves `receipt_for(frame)` requires matching request id, payload hash, command id, and command hash through `frame.matches_receipt(...)`; proves `receipt_for_request_id(request_id)` and `contains_request_id(request_id)` remain request-id-only; proves `has_conflicting_request(frame)` is true for same request id with different payload hash and false for different request id; and performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test api_transport_ledger_lookup_helper_preserves_frame_and_request_boundaries -- --test-threads=1`.

106. [x] `SCORE_REPORT.md`: after items 104 and 105 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows including `ai` and any changed function counts, and `score.md` changes only if refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.


Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 98 through 100 are complete and that there was no first incomplete executable item before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; inspected candidate source and test surfaces in `src/agent/cycle.rs`; and checked the working tree through the project connector. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning selected graph operations `21edae3cb3c10f1b`, `96af74928d58e034`, and `447858122f87cb5a`, covering `agent::cycle::{compute_envelope_hash, compute_evidence_contract_hash, compute_submit_evidence_command_hash}`, as the next safe non-`root_validate` consolidation area. The safe subset is private ordered submit-evidence hash input delegation only: keep all three private wrapper names and signatures, preserve distinct `HashDomain` values, domain seeds, prefix fields, field order, `h.max(1)` non-zero behavior, command JSON output, route tables, packet effects, worker API behavior, router behavior, runtime phase behavior, and `root_validate` unchanged.

101. [x] `src/agent/cycle.rs`: route `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, and `compute_envelope_hash(...)` through one private ordered submit-evidence hash-input helper while preserving distinct hash domains.
   - Scope: `src/agent/cycle.rs` only; allowed production functions are `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, `compute_envelope_hash(...)`, `compute_hash_domain_vector(...)`, `compute_domain_contract_hash(...)`, and at most one new private helper adjacent to them. Do not change `HashDomain` variants, domain seeds, prefix fields, `mix_contract_hash(...)`, `compute_structural_payload_hash(...)`, `build_submit_evidence_json(...)`, gate/evidence/effect route tables, worker/router clients, runtime phase dispatch, prompt code, tests, or `root_validate` in this item.
   - Done when: the evidence-contract wrapper still hashes `[gate_u64, evidence_u64, passed as u64, effect_u64, payload_hash]` in `HashDomain::EvidenceContract`; the submit-evidence command wrapper still hashes `[sub_contract_hash]` in `HashDomain::SubmitEvidenceCommand`; the command-envelope wrapper still hashes `[command_id, cmd_contract_hash]` in `HashDomain::CommandEnvelope`; all three wrappers delegate through the same private helper boundary; and no public API, JSON wire format, command hash, receipt, router, worker, filesystem, process, or network behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

102. [x] `src/agent/cycle.rs` test `submit_evidence_hash_input_helper_preserves_domain_vectors`: add focused regression coverage for the shared submit-evidence hash-input helper.
   - Scope: `src/agent/cycle.rs` test module only; use existing private helper access and existing submit-evidence hash test patterns. Do not change production code in this item.
   - Done when: the named test proves the Invariant/InvariantProof and Plan/TaskReady hash chains still produce the existing structural payload hashes, evidence-contract hashes, command hashes, envelope hashes, and `build_submit_evidence_json(...)` command-hash fields; proves the three hash domains remain non-zero and pairwise distinct for the same scalar input; proves field-order sensitivity for evidence-contract and command-envelope inputs; and performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test submit_evidence_hash_input_helper_preserves_domain_vectors -- --test-threads=1`.

103. [x] `SCORE_REPORT.md`: after items 101 and 102 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows including `ai` and any changed function counts, and `score.md` changes only if refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.


Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 92 through 94 are complete and that there was no first incomplete executable item before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`; inspected candidate source and test surfaces in `src/codec/ndjson.rs` and `src/capability/llm/openai.rs`; and checked the working tree through the project connector. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,570 planned operations. Current planning first inspected the high-volume `codec::ndjson` enum-decoder merge cluster, but rejected it as stale/no-op because `src/codec/ndjson.rs` already routes the enum tag decoders through `enum_from_u64(...)` and `decode_enum_from_u64!`. Current planning selected graph operation `4823eaed82cc2de7`, covering `capability::llm::openai::{encode_openai_judgment_proof_event_ndjson, encode_openai_llm_effect_receipt_ndjson}`, as the next safe non-`root_validate` consolidation area. The safe subset is private numeric-field NDJSON array formatting delegation only: keep both public encoder names and signatures, preserve schema-version fields, record-type fields, field order, numeric casts, bracketed comma-separated output, decoder round trips, append validation, proof/receipt hashing, replay verification, HTTP/client code, retry logic, request/response parsing, Ollama code, and `root_validate` unchanged.

95. [x] `src/capability/llm/openai.rs`: route `encode_openai_llm_effect_receipt_ndjson(...)` and `encode_openai_judgment_proof_event_ndjson(...)` through one private numeric-field NDJSON array formatting helper.
   - Scope: `src/capability/llm/openai.rs` only; allowed production functions are `encode_openai_llm_effect_receipt_ndjson(...)`, `encode_openai_judgment_proof_event_ndjson(...)`, and at most one new private helper adjacent to them. Do not change append functions, decode functions, load functions, verification functions, receipt/proof structs, receipt/proof hashing, replay verification, API/TLog call sites, HTTP/client code, retry logic, request/response parsing, Ollama code, graph mutation code, or tests in this item.
   - Done when: the receipt encoder still emits a 25-field bracketed comma-separated numeric JSON array with `OPENAI_LLM_EFFECT_RECEIPT_SCHEMA_VERSION` and `OPENAI_LLM_EFFECT_RECEIPT_RECORD` first; the proof-event encoder still emits a 23-field bracketed comma-separated numeric JSON array with `OPENAI_JUDGMENT_PROOF_SCHEMA_VERSION` and `OPENAI_JUDGMENT_PROOF_RECORD` first; boolean and small integer casts remain unchanged; both public encoders delegate through the same private helper boundary; and no public API, wire format, receipt/proof, replay, filesystem, or network behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

96. [x] `src/capability/llm/openai.rs` test `openai_ndjson_encoders_preserve_record_layouts`: add focused regression coverage for the shared OpenAI NDJSON encoder helper.
   - Scope: `src/capability/llm/openai.rs` test module only; use existing `OpenAiLlmEffectReceipt`, `OpenAiJudgmentProofEvent`, encoder, decoder, and deterministic hash/test helpers where available. Do not change production code in this item.
...
92. [x] `src/agent/loop_driver.rs`: route `agent_identity(...)` and `agent_tag(...)` through one private agent-label formatting helper while preserving prompt identity and log-tag semantics.
...
93. [x] `src/agent/loop_driver.rs` test `agent_label_helpers_preserve_identity_and_tag_boundaries`: add focused regression coverage for the shared agent-label helper.
...
94. [x] `SCORE_REPORT.md`: after items 92 and 93 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `scripts/recapture_rustc_graphs.sh --check` validates the configured graph root, `SCORE_REPORT.md` is regenerated from `../state/rustc`, `status.md` records aggregate and affected crate rows including `ai` and any changed function counts, and `score.md` changes only if refreshed evidence differs from the current rationale.
   - Validation: `bash scripts/recapture_rustc_graphs.sh --check && cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root ../state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
