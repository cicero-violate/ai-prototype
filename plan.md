...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 113 through 115 are complete and that no unchecked executable item remains before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; skipped stale already-reconciled non-`root_validate` candidates in `api::server::{evidence_from_str,gate_from_str}`, `capability::eval::record::{dimension_hash,dimension_order_hash,dimension_score_hash}`, and the already-completed policy-reuse content hash helpers; inspected candidate source and tests in `src/capability/judgment/record.rs`; and checked current graph-derived structural evidence in `SCORE_REPORT.md`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,566 planned operations. Current planning selected graph operation `7f0881931a92916d`, covering `capability::judgment::record::{policy_reuse_receipt_hash, policy_reuse_trend_receipt_hash}`, as the next safe non-`root_validate` consolidation area. The safe subset is private ordered policy-reuse receipt hash folding only: keep both function names and signatures, preserve each distinct domain seed, preserve all zero-hash and string-code validity guards, preserve every folded field and field order, preserve non-zero `h.max(1)` behavior through the existing `fold_ordered_policy_reuse_receipt_hash(...)` helper, preserve existing ledger-summary and scale-trace helper users, and leave `root_validate` unchanged.

116. [x] `src/capability/judgment/record.rs`: route `policy_reuse_receipt_hash(...)` and `policy_reuse_trend_receipt_hash(...)` through existing private `fold_ordered_policy_reuse_receipt_hash(...)` while preserving distinct current-policy and trend receipt domains.
   - Scope: `src/capability/judgment/record.rs` only; allowed production functions are `policy_reuse_receipt_hash(...)` and `policy_reuse_trend_receipt_hash(...)`. Do not change `PolicyReuseReceipt`, `PolicyReuseTrendReceipt`, `PolicyReuseLedgerSummaryReceipt`, `PolicyReuseScaleTraceReceipt`, `PolicyReusePerformanceCostTrendReceipt`, `PolicyReuseCostCatalogReceipt`, `PolicyReuseEvaluatorSavingsReceipt`, policy judgment records, JSON encoders, constructors, validation methods, API code, LLM code, or `root_validate` in this item.
   - Done when: `policy_reuse_receipt_hash(...)` still rejects zero schema or record-set hashes and invalid verdict codes; still uses seed `0x504f_4c52_4350_5448u64`; still folds schema version, retained-record count, policy-hit count, policy-miss count, avoided-LLM-call count, hit-rate bps, record-set hash, and verdict code in the existing order; `policy_reuse_trend_receipt_hash(...)` still rejects zero schema, baseline receipt hash, or current receipt hash and invalid trend or verdict codes; still uses seed `0x504f_4c52_5452_4e44u64`; still folds schema version, baseline receipt hash, current receipt hash, baseline retained-record count, current retained-record count, baseline hit-rate bps, current hit-rate bps, hit-rate delta bps, avoided-LLM-call delta, trend code, and verdict code in the existing order; both functions delegate through `fold_ordered_policy_reuse_receipt_hash(...)`; and no public API, receipt, replay, JSON, policy, filesystem, process, or network behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

117. [x] `src/capability/judgment/record.rs` test `policy_reuse_receipt_hash_helper_preserves_current_and_trend_boundaries`: add focused regression coverage for the shared policy-reuse receipt hash helper boundary.
   - Scope: `src/capability/judgment/record.rs` test module only; use existing `PolicyReuseReceipt`, `PolicyReuseTrendReceipt`, and policy-reuse fixture/constructor helpers already available in the test module when possible. Do not change production code in this item.
   - Done when: the named test proves current-policy and trend receipts remain valid; their receipt hashes are non-zero and distinct for related policy-reuse evidence; current-policy record-set, hit-rate, and verdict tampering are rejected or hash-bound; trend baseline/current receipt hash, hit-rate delta, avoided-call delta, trend status, and verdict tampering are rejected or hash-bound; and the test performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test policy_reuse_receipt_hash_helper_preserves_current_and_trend_boundaries -- --test-threads=1`.

118. [x] `SCORE_REPORT.md`: after items 116 and 117 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
