...
110. [ ] `src/capability/llm/record.rs`: route `prompt_hash(...)`, `response_hash(...)`, `rationale_hash(...)`, and `llm_payload_hash(...)` through one private ordered hash-folding helper while preserving each semantic hash domain.
