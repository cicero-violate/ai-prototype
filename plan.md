...
25. [ ] `src/capability/tooling/record/process.rs`: route `LiveSandboxProcessExecutor::{with_allowed_command, with_locked_env}` through one private builder mutation helper while preserving builder semantics.
...
26. [ ] `src/capability/tooling/record/process.rs` test `live_sandbox_process_executor_builders_preserve_policy_boundaries`: add focused regression coverage for the shared builder mutation helper.
