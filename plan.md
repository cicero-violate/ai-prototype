...
101. [ ] `src/agent/cycle.rs`: route `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, and `compute_envelope_hash(...)` through one private ordered submit-evidence hash-input helper while preserving distinct hash domains.
