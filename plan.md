...
43. [ ] `src/capability/llm/ollama.rs`: route `OllamaLlmEffectReceipt::{canonical_authority_hash, canonical_request_hash}` through one private ordered hash-fold helper while preserving distinct authority and request domains.
...
44. [ ] `src/lib.rs` test `ollama_effect_receipt_hash_helpers_preserve_authority_request_domains`: add focused regression coverage for the shared effect-receipt hash helper.
