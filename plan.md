...
122. [ ] `src/capability/llm/ollama.rs`: route `load_ollama_llm_effect_receipts_ndjson(...)`, `load_ollama_llm_effect_receipts_ndjson_unchecked(...)`, and `load_ollama_judgment_proof_events_ndjson(...)` through one private typed Ollama NDJSON loader helper while preserving checked and unchecked decoder boundaries.
