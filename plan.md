...
22. [ ] `src/capability/observation/source.rs`: route `ObservationIngressBatch::{empty, backpressure, rejected}` through one private constructor helper while preserving distinct ingress decisions.
...
23. [ ] `src/capability/observation/source.rs` test `observation_ingress_batch_constructors_preserve_decision_boundaries`: add focused regression coverage for the shared non-accepted ingress batch constructor helper.
