...
Planning reconnaissance on 2026-05-12 confirms items 60 through 108 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 109, focused loopback/unit coverage for the extracted `src/agent/router.rs::open_streaming_http_stream(...)` helper. Current `SplitFn id=c07f9b3fef3c6e37` for `agent::router::collect_streaming_response_bytes` (`expected_lo=16668`, `expected_hi=19103`, generated names `collect_streaming_response_bytes__parse`/`collect_streaming_response_bytes__transform`) is historical evidence after item 108 extracted the safe manual socket setup/request write helper; it is not an instruction to split again before coverage and score refresh. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only. The legitimate current source target is now the `src/agent/router.rs` test module, because Structure remains the lowest graph-derived axis and item 108 added a new private helper that needs local non-external coverage. The safe item-109 test should use loopback or equivalent in-process fixtures to prove exact request bytes are sent without changing response accumulation, chunk logging, `[DONE]` detection, stream-deadline handling, EOF behavior, final HTTP parsing, or public `OpenAiError` semantics.
