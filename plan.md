...
Planning reconnaissance on 2026-05-12 confirms items 60 through 75 are complete with targeted and broader validation as recorded below. The next executable item is item 77, a graph-backed helper extraction in `src/agent/loop_driver.rs::sync_mcp_workspace(...)` that isolates HTTP response status parsing while preserving endpoint parsing, request writing, response reading, and error behavior. Item 74 inspected `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` and confirmed `SplitFn id=8d96a0680113da93` for `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process` with `expected_lo=6609`, `expected_hi=8939`, generated names `execute_authorized_process__parse` and `execute_authorized_process__transform`, and `delegate_strategy=preserve_original_signature`. Source inspection found the smallest safe helper boundary is the timeout/try-wait loop inside `execute_authorized_process(...)`, with existing sandbox process receipt tests available through `cargo test live_sandbox_process_runner_records_and_replays_receipt -- --test-threads=1`. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
77. [ ] `src/agent/loop_driver.rs`: extract HTTP response status parsing from `sync_mcp_workspace(...)` into private helper `parse_mcp_workspace_status(response: &str) -> u16` while preserving `sync_mcp_workspace(mcp_url, project_dir) -> Result<(), String>` behavior.
