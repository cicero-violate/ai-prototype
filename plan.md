...
Planning reconnaissance on 2026-05-12 confirms items 60 through 104 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 105, focused in-memory error coverage for `finalize_streaming_response(...)`, followed by graph-derived evidence refresh in item 106 and a post-refresh router byte-collector split inspection in item 107. Current `SplitFn id=c07f9b3fef3c6e37` for `agent::router::collect_streaming_response_bytes` (`expected_lo=16668`, `expected_hi=19103`, generated names `collect_streaming_response_bytes__parse`/`collect_streaming_response_bytes__transform`) is the live graph-backed follow-on candidate after item 102 extracted the helper; it is evidence only, not an instruction to split again before finalization tests. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only. The legitimate current source target is `src/agent/router.rs::finalize_streaming_response(...)` error-path test coverage, because Structure remains the lowest graph-derived axis and tests around the new transport/finalization boundary provide validation evidence before any further graph-backed refactor.
...
105. [ ] `src/agent/router.rs`: add in-memory error coverage for `finalize_streaming_response(...)` non-200 and missing-`[DONE]` paths.
   - Scope: `src/agent/router.rs` test module only. Do not open sockets, call live router endpoints, or edit production transport logic.
...
106. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after items 101-105 land, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
...
107. [ ] `src/agent/router.rs`: inspect live graph-backed `agent::router::collect_streaming_response_bytes(...)` and record whether a smaller safe helper boundary exists after item 105 error tests and item 106 graph refresh.
   ...
   - Done when: inspection confirms whether the current `SplitFn id=c07f9b3fef3c6e37` target still exists, rejects generated parse/transform names as direct instructions, and records a manual file-level boundary such as isolating socket setup, request write/flush, read-loop accumulation, or done-frame deadline handling only if behavior and public `OpenAiError` semantics stay unchanged.
   - Validation: `cargo check`.
