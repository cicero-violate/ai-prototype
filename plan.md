...
Planning reconnaissance on 2026-05-11 confirms that the first incomplete executable item remains item 58, `src/agent/loop_driver.rs::LoopDriver::run_cycle` validation for the existing `run_cycle_attempt(...)` helper extraction. Source inspection found `run_cycle` at `src/agent/loop_driver.rs:129`, `run_cycle_attempt` at `src/agent/loop_driver.rs:238`, `RunCycleAttemptOutcome` at `src/agent/loop_driver.rs:365`, and `retry_attempt_label` at `src/agent/loop_driver.rs:684`. The graph-derived objective still favors small helper extractions because `SCORE_REPORT.md` reports Structure as the lowest aggregate structural axis at `4.8`. Do not advance to `sync_mcp_workspace`, `AgentCycle::run`, router, graph mutation, or process-executor split candidates until item 58 has green validation evidence or a new blocker entry explains why it cannot be completed.
...
58. [ ] `src/agent/loop_driver.rs::LoopDriver::run_cycle`: validate the existing `run_cycle_attempt(...)` helper extraction and mark it complete only after green full-suite Rust output.
   - Scope: `src/agent/loop_driver.rs` only for the item 58 extraction. Current source inspection shows `run_cycle_attempt(...)` exists and delegates the per-attempt `ChunkLogger`, `OpenAiChatRequest`, and `RouterClient::streaming_turn` block; do not mark complete from source inspection alone. Do not edit `sync_mcp_workspace`, graph mutation code, domain code, tests, fixtures, generated auto-refactor JSON, or unrelated router/certification changes while selecting this item.
   ...
   - Validation: `cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
59. [ ] `src/agent/loop_driver.rs`: after item 58 is green, inspect graph-backed `SplitFn id=d535999f445621fb` for `agent::loop_driver::sync_mcp_workspace` and write the smallest helper-extraction implementation checklist item for that exact function.
   - Scope: `plan.md`, `status.md`, and `src/agent/loop_driver.rs` inspection only; do not edit Rust source in this planning item.
   ...
   - Validation: `rg -n "fn sync_mcp_workspace|sync_mcp_workspace\(" src/agent/loop_driver.rs` and planning/status review.
