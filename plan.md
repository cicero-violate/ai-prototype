...
Planning reconnaissance on 2026-05-13 confirms items 60 through 139 are complete with targeted validation, reconciliation evidence, or graph-derived score refresh evidence as recorded below. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next executable work should prefer small, graph-backed decomposition with validation evidence. The first incomplete Active Priorities item is item 140, `../chatgpt-mcp-connector/src/tools.rs` focused regression coverage for the item-139 policy-selected evaluator transport input helper. Current auto-refactor evidence reports no remaining operations for the recent completed patch helper SplitFn targets; the remaining explicit graph-backed SplitFn candidates are in `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`, with the next legitimate SplitFn operations after the completed `execute_policy_selected_evaluator_suite_tool_inner(...)` extraction including `tools::execute_evaluator_suite_tool_inner(...)` and `tools::execute_durable_policy_selected_suite(...)`. Do not apply generated names or merge-surface recommendations blindly.
