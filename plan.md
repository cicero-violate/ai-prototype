...
16. [ ] `src/capability/llm/openai.rs`: route `json_string_field(...)` and `message_content_field(...)` through one private scoped string-field lookup helper.
...
17. [ ] `src/capability/llm/openai.rs` test `openai_chat_response_parser_preserves_message_scoped_content_fields`: add regression coverage for scoped content extraction after the shared parser helper.
