...
Planning reconnaissance on 2026-05-12 confirms items 60 through 68 are complete with targeted and broader validation as recorded below. The first incomplete executable item is item 69, `src/agent/router.rs::build_streaming_http_request(...)`, selected from graph-backed `SplitFn id=1ea38f3cc5f37f85` for `agent::router::send_streaming_request` in `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json`. This target is smaller and lower-risk than `graph_mutation::generate_graph_patch`, `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process`, or `runtime::introspection::introspect_canonical_tlog`, and it directly improves call-structure evidence while Structure remains the lowest graph-derived axis in `SCORE_REPORT.md`.
...
69. [ ] `src/agent/router.rs`: extract streaming HTTP request-string construction from `send_streaming_request(...)` into private helper `build_streaming_http_request(path: &str, host: &str, port: u16, body: &str) -> String` while preserving `send_streaming_request(config, body, logger, stream_deadline_ms) -> Result<crate::agent::sse::SseResult, OpenAiError>`.
   - Scope: `src/agent/router.rs` only. Allowed edits are `send_streaming_request(...)`, the new private helper, and a focused unit test in the existing `#[cfg(test)] mod tests`; do not edit endpoint parsing, TCP connection, timeout handling, SSE parsing, chunked decoding, retry policy, browser-tab behavior, generated auto-refactor JSON, domain code, graph mutation code, or unrelated runtime/source files.
   - Done when: `send_streaming_request(...)` still parses the local endpoint, computes `chat_completions_path(...)`, connects and sets read/write timeouts, writes and flushes the helper-produced bytes, preserves deadline/read loop behavior, preserves HTTP status and `[DONE]` checks, and preserves chunked/non-chunked SSE body parsing.
