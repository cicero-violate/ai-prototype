...
158. [ ] `src/bin/agent.rs`: extract the private helper selected by item 157 from `supervisor_reload_worker_port(...)` without changing supervisor reload semantics.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --bin agent`.
