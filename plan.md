...
31. [ ] `src/capability/llm/ollama.rs`: route `OllamaJudgmentProofEvent::{expected_proof_hash, verifier_context_hash}` through one private ordered hash-fold helper while preserving distinct hash domains.
...
32. [ ] `src/lib.rs` test `ollama_proof_event_hash_helpers_preserve_distinct_domains`: add focused regression coverage for the shared proof-event hash helper.
