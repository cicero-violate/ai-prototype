...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and the source/test surfaces named below. There was no first incomplete executable item before this planning turn: prior Active Priorities items 16 through 18 were complete and `status.md` confirmed item exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan is schema version 1 and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle` phase-route/recovery-route helper tables, `agent::objective` hash setters, `agent::prompt` certification prompt formatting, `api::server` submission token parsing, `agent::cycle` hash-domain wrapper extraction plus descriptor regression coverage, `agent::router::{env_u32, env_u64}` retry parser consolidation plus parser regression coverage, `agent::worker_client::{WorkerClient::new, WorkerClient::new_with_timeout}` constructor delegation plus constructor regression coverage, `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}` request-id lookup delegation plus direct conflict regression coverage, `api::transport::ApiTransportReceipt::{new, is_contract_valid}` expected-hash helper coverage, and `api::transport::ApiTransportSession::{from_parts, verify}` shared verifier plus direct tampered-receipt session coverage. Several remaining `agent::cycle`, prompt, evidence, transport hash, and receipt hash candidates are structurally similar but semantically unsafe to merge wholesale because the names document distinct envelope, contract, command, gate, evidence, effect, phase, prompt, transport-frame, and receipt-hash domains. Graph operation rank 54 identifies `capability::CapabilityEffectRoute::{allows, permits_effect}` as the next safe non-root consolidation area because both check the same route tuple. The safe subset is to make `CapabilityEffectRoute::allows(...)` delegate the capability/gate/evidence/effect tuple check to `permits_effect(...)` for passed submissions, while preserving the existing failed-submission rule that only `PacketEffect::None` is allowed. Existing registry tests cover canonical tool effects and denial paths; the plan adds direct route-level coverage for passed, failed, wrong-effect, and wrong-capability submissions before refreshing graph evidence.

19. [x] `src/capability/mod.rs`: make `CapabilityEffectRoute::allows(...)` delegate the passed-submission tuple check through `CapabilityEffectRoute::permits_effect(...)`.
   - Scope: `src/capability/mod.rs` `impl CapabilityEffectRoute` only; allowed functions are `CapabilityEffectRoute::allows(...)` and, only if needed, private local helper code inside the same impl. Do not change `CapabilityEffectRoute::new(...)`, `CapabilityEffectRoute::permits_effect(...)`, `CapabilityRegistry::{allows, permits_effect}`, route constants, policy hashes, public signatures, or packet effect semantics.
