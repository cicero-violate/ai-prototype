...
153. [ ] `src/bin/root_validate.rs`: add unit test `root_validate_dispatch_catalog_lists_every_compact_mode_once`.
   ...
   - Current planning evidence: 2026-05-13 planning reconnaissance reconfirmed this as the first incomplete executable item. Local source scan finds the relevant `src/bin/root_validate.rs` symbols at lines 37 (`COMPACT_MODES`), 775-776 (dispatch-catalog marker and runner entry), 1486 (`root_validate_dispatch_catalog_payload()`), 2399 (`root_validate_dispatch_catalog_mode()`), 2421 (`try_run_compact_mode(...)`), 2428 (`compact_mode_stdout_for_contract(...)`), and 2493 (`mirror_validation_outcome(...)`). `state/rustc/auto-refactor`, `state/rustc/root_validate__bin/graph.json`, `state/rustc/root_validate/graph.json`, and `state/rustc/ai/graph.json` are absent, so execute this item as a test-only change unless source inspection proves a smaller helper is required. The current working tree has unrelated modifications outside `src/bin/root_validate.rs`; do not include them in item 153.
