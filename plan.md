...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and the source/test surfaces named below. There was no first incomplete executable item before this planning turn: prior Active Priorities items 31 through 33 were complete and `status.md` confirmed item exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle` phase-route/recovery-route helper tables, `agent::objective` hash setters, `agent::prompt` certification prompt formatting, `api::server` submission token parsing, `agent::cycle` hash-domain wrapper extraction plus descriptor regression coverage, `agent::router::{env_u32, env_u64}` retry parser consolidation plus parser regression coverage, `agent::worker_client::{WorkerClient::new, WorkerClient::new_with_timeout}` constructor delegation plus constructor regression coverage, `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}` request-id lookup delegation plus direct conflict regression coverage, `api::transport::ApiTransportReceipt::{new, is_contract_valid}` expected-hash helper coverage, `api::transport::ApiTransportSession::{from_parts, verify}` shared verifier plus direct tampered-receipt session coverage, `capability::CapabilityEffectRoute::{allows, permits_effect}` passed-submission delegation plus failed-submission regression coverage, `graph_mutation::GraphMutationOpRow::{is_self_consistent, new}` row invariant helper plus tampered-field regression coverage, `capability::EvidenceSubmission::{new, with_effect}` constructor delegation plus constructor-equivalence coverage, and `capability::judgment::record::PolicyJudgmentRecord::{decision, judgment_record}` shared-validity helper plus boundary regression coverage. Several remaining `agent::cycle`, prompt, transport-frame, receipt encoder, policy-reuse, LLM message, and domain-specific hash candidates are structurally similar but semantically unsafe to merge wholesale because the names document distinct envelope, contract, command, gate, evidence, effect, phase, prompt, transport-frame, graph-receipt, policy, trend, role, and receipt-hash domains.

Graph operation `999e48d20cebd1f5` identifies `capability::judgment::record::PolicyReuseCostCatalogReceipt::{from_source_hashes, is_valid}` as the next safe non-root consolidation area. The safe subset is not to merge constructor and validator semantics or alter catalog coverage rules. Instead, extract private helpers for the catalog completion predicate and source-hash presence checks, use them from both `from_source_hashes(...)` and `is_valid(...)`, and keep catalog/receipt hash functions and public fields unchanged. Existing tests cover complete and incomplete catalog receipts; the plan adds direct regression coverage for the shared source-hash and completion boundary before refreshing graph evidence.

Planning reconnaissance refresh on 2026-05-14 after item 36 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`, and the source/test surfaces named below. There was no first incomplete executable item before this planning turn: Active Priorities items 34 through 36 were complete and `status.md` confirmed item exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence remains `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan remains schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `capability::EvidenceSubmission::{new, with_effect}`, `capability::eval::record::{dimension_order_hash, dimension_score_hash}`, `capability::judgment::record::PolicyJudgmentRecord::{decision, judgment_record}`, and `capability::judgment::record::PolicyReuseCostCatalogReceipt::{from_source_hashes, is_valid}`. Adjacent remaining judgment hash, policy-reuse receipt hash, LLM role-message, and receipt encoder candidates remain rejected for this turn because structurally similar wrappers represent distinct semantic domains.

Graph operations `50696c68b2277059` and `72dcf1b2a173f2c7` identify `capability::learning::promote::<impl PolicyStore>::{promote, promote_feedback}` and `{promote_durable, promote_feedback_durable}` as the next safe non-root consolidation area. The safe subset is private helper extraction for `PolicyEntry` construction and checked append dispatch, not a merge of public methods. `promote(...)` and `promote_durable(...)` must continue writing `POLICY_PROMOTION_SOURCE_SEQ` with `promotion.source_seq`; `promote_feedback(...)` and `promote_feedback_durable(...)` must continue writing `POLICY_FEEDBACK_HASH` with `promotion.promoted_policy_hash`; all four methods must continue rejecting invalid promotions with `PolicyStoreError::InvalidPromotion` before appending.

28. [x] `src/capability/eval/record.rs`: extract shared private dimension-hash iteration while preserving separate eval hash domains.
   - Scope: `src/capability/eval/record.rs` private helpers only; allowed functions are `dimension_order_hash(...)`, `dimension_score_hash(...)`, and a new private helper/type such as `dimension_hash(...)` plus `DimensionHashMode`. Do not change `eval_payload_hash(...)`, `dimension_id_hash(...)`, `EvalRecord`, `EvalScorecardReceipt`, NDJSON encode/decode/load/append behavior, public signatures, receipt hash semantics, verdict semantics, or evidence submission behavior.
...
31. [x] `src/capability/judgment/record.rs`: extract a private `PolicyJudgmentRecord` validity-to-`JudgmentRecord` helper for `decision(...)` and `judgment_record(...)`.
   - Scope: `src/capability/judgment/record.rs` only; allowed functions are `PolicyJudgmentRecord::decision(...)`, `PolicyJudgmentRecord::judgment_record(...)`, and a new private helper such as `policy_judgment_hit_record(...)`. Do not change `PolicyJudgmentRecord::from_context_policy(...)`, `PolicyJudgmentRecord::from_context_policy_lookup_receipt(...)`, `PolicyJudgmentRecord::is_valid(...)`, `PolicyJudgmentRecord::submission(...)`, `JudgmentRecord`, `judgment_payload_hash(...)`, `policy_judgment_record_hash(...)`, public signatures, receipt hashes, policy lookup semantics, or evidence submission behavior.
...
34. [x] `src/capability/judgment/record.rs`: extract private `PolicyReuseCostCatalogReceipt` catalog-completion and source-hash helper predicates for `from_source_hashes(...)` and `is_valid(...)`.
   - Scope: `src/capability/judgment/record.rs` only; allowed functions are `PolicyReuseCostCatalogReceipt::from_source_hashes(...)`, `PolicyReuseCostCatalogReceipt::is_valid(...)`, and new private helpers such as `policy_reuse_cost_catalog_summary_complete(...)` and `policy_reuse_cost_catalog_sources_present(...)`. Do not change public struct fields, public signatures, `PolicyReuseCostCatalogReceipt::passed(...)`, `PolicyReuseCostCatalogReceipt::to_json(...)`, `policy_reuse_cost_catalog_content_hash(...)`, `policy_reuse_cost_catalog_receipt_hash(...)`, evaluator-savings behavior, receipt hashes, or accepted record type / missing-mode values.
   - Done when: `from_source_hashes(...)` derives `summary_complete` through the shared completion helper, `is_valid(...)` uses the same completion helper and the shared nonzero-source helper, and existing complete/incomplete/tampered catalog behavior remains observable.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

35. [x] `src/capability/judgment/record.rs` test `policy_reuse_cost_catalog_helper_boundary_rejects_source_and_completion_tampering`: add direct regression coverage for the shared catalog helper boundary.
   - Scope: `src/capability/judgment/record.rs` test module only.
...
37. [x] `src/capability/learning/promote.rs`: extract private policy-promotion entry builder and checked in-memory append helper for `PolicyStore::promote(...)` and `PolicyStore::promote_feedback(...)`.
   - Scope: `src/capability/learning/promote.rs` only; allowed methods/functions are `PolicyStore::promote(...)`, `PolicyStore::promote_feedback(...)`, and new private helpers such as `policy_promotion_entry(...)` and `append_valid_promotion_entry(...)`. Do not change `PolicyPromotion::from_tlog(...)`, `PolicyPromotion::is_valid(...)`, `PolicyPromotion::submission(...)`, `PolicyStore::try_append(...)`, `PolicyStore::append_durable(...)`, policy constants, public signatures, durable methods, distillation export behavior, or evidence submission behavior.
