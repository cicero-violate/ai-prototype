...
Planning reconnaissance on 2026-05-13 confirms items 60 through 138 are complete with targeted validation, reconciliation evidence, or graph-derived score refresh evidence as recorded below. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next executable work should prefer small, graph-backed decomposition with validation evidence. The first incomplete Active Priorities item is item 139, `../chatgpt-mcp-connector/src/tools.rs::execute_policy_selected_evaluator_suite_tool_inner(...)` helper extraction for policy-selected evaluator transport input parsing and suite/policy lookup preparation. Current auto-refactor evidence reports no remaining operations for the recent completed patch helper SplitFn targets; the remaining explicit graph-backed SplitFn candidates are in `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`, with the next legitimate SplitFn operation after the completed `check_effectful_patch(...)` extraction targeting `../chatgpt-mcp-connector/src/tools.rs::execute_policy_selected_evaluator_suite_tool_inner(...)`. Do not apply generated names or merge-surface recommendations blindly.
...
140. [ ] `../chatgpt-mcp-connector/src/tools.rs`: add or tighten focused test coverage for the item-139 policy-selected evaluator transport input helper without changing evaluator execution semantics.
   - Scope: `../chatgpt-mcp-connector/src/tools.rs` test module only. Prefer existing `tools::tests::canon_execute_policy_selected_evaluator_suite_*` coverage when sufficient; otherwise add one focused test around caller-policy rejection, default internal intent selection, or allowed-program parsing as exposed through `canon_execute_policy_selected_evaluator_suite`.
   - Done when: the helper boundary introduced by item 139 has direct regression coverage and the public MCP response shape remains unchanged.
   - Validation: `cargo test --manifest-path ../chatgpt-mcp-connector/Cargo.toml tools::tests::canon_execute_policy_selected_evaluator_suite -- --test-threads=1`.
