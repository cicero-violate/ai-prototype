...
140. [ ] `src/api/server.rs`: route `gate_from_str(...)` and `evidence_from_str(...)` through one private typed API submission-token extraction helper while preserving distinct gate and evidence domains.
