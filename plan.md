...
1. [ ] `src/api/server.rs`: consolidate `gate_from_str(...)` and `evidence_from_str(...)` behind one private API submission token parser while preserving all accepted string literals, enum outputs, and invalid-payload errors.
