...
Graph operations for `capability::llm::openai::OpenAiMessage::{assistant, system}`, `{assistant, tool}`, `{assistant, user}`, `{system, tool}`, `{system, user}`, and `{tool, user}` identify `OpenAiMessage` constructors as the next safe non-root consolidation area. The safe subset is private constructor helper delegation only: public role constructors may route through one private helper that sets `role`, `content`, `tool_call_id`, and `tool_calls` exactly as before. Do not collapse the public constructors into one semantic API: `system(...)`, `user(...)`, `assistant(...)`, `assistant_tool_call(...)`, and `tool(...)` remain distinct public message-boundary constructors. Preserve public signatures, role strings, optional content behavior, tool-call-id behavior, tool-call vector behavior, `message_contract_valid(...)`, `push_message_json(...)`, chat request JSON behavior, context message generation, HTTP/client code, retry-budget behavior, receipt/proof behavior, NDJSON behavior, and all network surfaces.

13. [ ] `src/capability/llm/openai.rs`: route `OpenAiMessage::{system, user, assistant, assistant_tool_call, tool}` through one private constructor helper.
...
14. [ ] `src/capability/llm/openai.rs` test `openai_message_constructors_preserve_role_boundaries`: add regression coverage for the shared message constructor helper.
