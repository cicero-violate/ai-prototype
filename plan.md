...
Planning reconnaissance on 2026-05-12 confirms items 60 through 87 are complete with targeted and broader validation as recorded below. The next executable item is item 88, a graph-backed inspection of `examples/ollama_tool_loop_trace.rs::submit_ollama_tool_calls(...)`. Current `SplitFn id=a0f85bbd8f5f9cf1` for `submit_ollama_tool_calls` is legitimate graph-backed evidence because the example still contains a long tool-call loop plus post-loop successful MCP receipt selection, `Command::SubmitEvidence`, `handle_envelope(...)`, and return-count finalization in one function. The proposed next helper boundary should mirror the already validated OpenAI item 87 pattern as closely as the MCP evidence model allows: post-loop combined MCP evidence submission only. Do not move Ollama request construction, LLM intent validation, MCP execution, per-call receipt persistence, tool-result mapping, or kernel/TLog authority into the helper. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
88. [ ] `examples/ollama_tool_loop_trace.rs`: inspect graph-backed `SplitFn id=a0f85bbd8f5f9cf1` for `submit_ollama_tool_calls` and record the smallest safe helper-extraction checklist item for the post-loop MCP evidence submission boundary in that exact function.
