...
Graph operation `fefb1237c6a1ab0a` identified `capability::llm::openai::{json_string_field, message_content_field}` and has now been reconciled by completed items 16 through 18. Current planning turn verified graph-backed prompt candidates `agent::prompt::{analysis_prompt, judgment_prompt, plan_prompt, eval_prompt, recovery_prompt}` as the next safe non-root consolidation area. The source tree already contained the private `certification_phase_prompt(...)` helper and wrapper delegation for item 19, and targeted `cargo check --lib` passed, so item 19 is marked complete with validation evidence. The next executable item is item 20, the missing named regression test that must preserve the five distinct public prompt identities, phase labels, context labels, instruction text, final-verdict/no-tool rules, and call-site behavior. Do not merge prompt identities, do not change `planning_prompt(...)`, and do not touch `src/agent/loop_driver.rs` because it has unrelated uncommitted edits.
