...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, `SCORE_REPORT.md`, all current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths, and the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence reports `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten.

The selected `ai` graph-editor plan is schema version 1 and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle` phase-route/recovery-route helper tables, `agent::objective` hash setters, `agent::prompt` certification prompt formatting, and `api::server` submission token parsing. Several remaining `agent::cycle` candidates are structurally similar but semantically unsafe to merge wholesale because the names document distinct envelope, contract, command, gate, evidence, effect, phase, and recovery concepts. The next legitimate small subset is graph operation id `21edae3cb3c10f1b`, followed by ids `96af74928d58e034` and `447858122f87cb5a`: `agent::cycle::{compute_evidence_contract_hash, compute_submit_evidence_command_hash, compute_envelope_hash}` already share `mix_contract_hash(...)` but still duplicate the wrapper pattern. The safe executable work is to introduce a typed local hash-domain descriptor so the wrappers remain semantically named while their seed/schema/field-shape mechanics are table-driven and regression-tested.

1. [x] `src/agent/cycle.rs`: replace duplicate command/envelope/contract hash wrapper mechanics with a typed local hash-domain descriptor while preserving public/private helper names and exact hash outputs.
   - Scope: `src/agent/cycle.rs` functions `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, `compute_envelope_hash(...)`, `mix_contract_hash(...)`, and any new private enum/struct/helper immediately supporting those functions only.
