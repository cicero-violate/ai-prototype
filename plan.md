...
57. [ ] `src/agent/loop_driver.rs`: after items 50, 53, 54, and 55 are complete, inspect graph-backed `SplitFn id=001e821dc83e940a` for `agent::loop_driver::LoopDriver::run_cycle` and write the smallest helper-extraction implementation checklist item for that exact function.
   ...
   - Validation: `rg -n "fn run_cycle|run_cycle\(" src/agent/loop_driver.rs` and planning/status review.
58. [ ] `src/agent/loop_driver.rs`: apply the first approved `LoopDriver::run_cycle` helper extraction from item 57 while preserving the original public behavior and signature.
