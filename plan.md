...
113. [ ] `src/capability/judgment/record.rs`: route `policy_reuse_cost_catalog_content_hash(...)` and `policy_reuse_evaluator_savings_content_hash(...)` through one private ordered policy-reuse content hash-fold helper while preserving distinct catalog and evaluator-savings domains.
