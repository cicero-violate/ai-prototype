...
Planning reconnaissance on 2026-05-12 confirms that item 60 has its source change and targeted test evidence: `src/agent/loop_driver.rs::sync_mcp_workspace` delegates endpoint parsing to `parse_mcp_workspace_endpoint(...)`, and `loop_driver::tests::mcp_workspace_endpoint_parser_preserves_host_port_and_errors` passed. Broader validation remains blocked by an unrelated rustfmt diff in `src/bin/agent.rs` around `supervisor_reload_worker_port(...)` HTTP 204 fallback handling. Items 61 and 62 now have green full-suite evidence. User direction on 2026-05-12 selects the nonblocking persistent-loop behavior: `LoopDriver::run_cycle(...)` must not synchronously close the browser tab after the loop. Item 63 now has source-presence, `cargo check`, and `cargo build --release` evidence for the user-selected nonblocking persistent-loop behavior. Item 64 inspection is complete for graph-backed `SplitFn id=918a1611235eccfd` on `agent::cycle::AgentCycle::run`. The first incomplete executable item is now item 65: extract the Step 0 planning-turn block in `src/agent/cycle.rs` into `run_planning_turn(...)` while preserving `AgentCycle::run(...)` behavior and public signature. Do not advance to router, graph mutation, process-executor, or additional `sync_mcp_workspace` split candidates until item 65 has targeted and full-suite validation evidence or a new blocker entry explains why it cannot be completed.
...
65. [ ] `src/agent/cycle.rs`: extract the Step 0 planning-turn block from `AgentCycle::run(...)` into a private method `run_planning_turn(&mut self, domain: &str, metric: &str) -> Result<(), CycleError>` while preserving the public `AgentCycle::run(&mut self) -> Result<AgentRunSummary, CycleError>` signature.
   ...
   - Validation: `cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test cycle::hash_tests -- --test-threads=1 && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
