...
Planning reconnaissance on 2026-05-12 confirms items 60 through 106 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 107, post-refresh router byte-collector split inspection. Current `SplitFn id=c07f9b3fef3c6e37` for `agent::router::collect_streaming_response_bytes` (`expected_lo=16668`, `expected_hi=19103`, generated names `collect_streaming_response_bytes__parse`/`collect_streaming_response_bytes__transform`) is the live graph-backed follow-on candidate after item 102 extracted the helper; it is evidence only, not an instruction to split again before finalization tests. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only. The legitimate current source target is now `src/agent/router.rs::collect_streaming_response_bytes(...)`, because Structure remains the lowest graph-derived axis and items 104-105 already added finalization success/error tests. The safe manual follow-on candidate is a narrow socket setup and request write/flush helper; response accumulation, chunk logging, `[DONE]` detection, stream-deadline handling, EOF behavior, final HTTP parsing, and public `OpenAiError` semantics must remain unchanged.
...
107. [ ] `src/agent/router.rs`: inspect live graph-backed `agent::router::collect_streaming_response_bytes(...)` and record the exact helper boundary for the next execution item.
   ...
   - Validation: `cargo check`.
108. [ ] `src/agent/router.rs`: extract private helper `open_streaming_http_stream(...)` from `collect_streaming_response_bytes(...)` for socket setup and request write/flush only.
   ...
   - Validation: `cargo check && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test router::tests -- --test-threads=1`.
109. [ ] `src/agent/router.rs`: add focused in-memory/unit coverage for the extracted `open_streaming_http_stream(...)` behavior without live router endpoints.
   ...
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test router::tests -- --test-threads=1`.
110. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after items 107-109 and review score rationale without changing project-level scores unless new capability evidence exists.
   - Scope: `SCORE_REPORT.md`, `score.md`, `status.md`, and `plan.md` only.
