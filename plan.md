...
Planning reconnaissance on 2026-05-12 confirms items 60 through 82 are complete with targeted and broader validation as recorded below. The next executable item is item 83, a helper extraction in `src/agent/router.rs::send_streaming_request(...)` around streaming response finalization only, preserving endpoint parsing, socket timeout setup, request writing, chunk logging, done-frame timeout behavior, HTTP status handling, chunked decoding, SSE parsing, and public `send_streaming_request(config, body, logger, stream_deadline_ms)` behavior. Item 80 inspected `SplitFn` evidence for `agent::loop_driver::LoopDriver::run_cycle`; item 81 then extracted `build_project_planning_prompt(...)` with passing targeted loop-driver prompt validation, `cargo fmt --check`, and full-suite validation. Item 82 inspected `SplitFn id=1ea38f3cc5f37f85` for `agent::router::send_streaming_request` and selected the post-read response-head/body parsing, HTTP status validation, done-frame check, chunked decoding fallback, and SSE parsing finalization block as the smallest safe extraction boundary. Generated `merge_surface` operations remain deferred and must be filtered manually.
