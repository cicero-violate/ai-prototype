...
Planning reconnaissance on 2026-05-12 confirms items 60 through 100 are complete with targeted and broader validation as recorded below. The next executable item is item 101, source/graph inspection for `src/agent/router.rs::send_streaming_request(...)` before a small streaming HTTP byte-collection helper extraction is attempted in item 102 and focused router tests are added in item 103. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only; the legitimate current source target is `src/agent/router.rs::send_streaming_request(...)`, where the safest manual boundary is streaming HTTP byte collection, not generated merge-surface edits. Generated `merge_surface` operations remain deferred and must be filtered manually. Planning reconnaissance on 2026-05-12 selected `src/agent/router.rs::send_streaming_request(...)` from the current `agent::router::send_streaming_request` split surface because Structure remains the lowest graph-derived axis and the source has a current, narrow transport byte-collection boundary.
