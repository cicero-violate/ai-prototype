...
Planning reconnaissance on 2026-05-12 confirms items 60 through 100 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item remains item 101, source/graph inspection for `src/agent/router.rs::send_streaming_request(...)` before a small streaming HTTP byte-collection helper extraction is attempted in item 102 and focused router tests are added in item 103. Current `SplitFn id=1ea38f3cc5f37f85` for `agent::router::send_streaming_request` (`expected_lo=15798`, `expected_hi=18665`, generated names `send_streaming_request__parse`/`send_streaming_request__transform`) is the live graph-backed candidate for this checklist. Current `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and the previously completed `LoopDriver::run_cycle`/`sync_mcp_workspace` candidates remain historical evidence only. The legitimate current source target is `src/agent/router.rs::send_streaming_request(...)`, where the safest manual boundary is streaming HTTP byte collection, not generated merge-surface edits. Generated `merge_surface` operations remain deferred and must be filtered manually. Planning reconnaissance on 2026-05-12 selected this target because Structure remains the lowest graph-derived axis and the source has a current, narrow transport byte-collection boundary.
...
101. [ ] `src/agent/router.rs`: inspect graph-backed `agent::router::send_streaming_request(...)` and record the smallest safe helper-extraction boundary for streaming HTTP transport byte collection.
   ...
   - Validation: `cargo check`.
102. [ ] `src/agent/router.rs`: extract streaming socket write/read/deadline byte collection from `send_streaming_request(...)` into one private helper such as `collect_streaming_response(endpoint_host: &str, endpoint_port: u16, write_timeout_ms: u64, request: &str, logger: &mut ChunkLogger, stream_deadline_ms: u64) -> Result<Vec<u8>, OpenAiError>`.
...
103. [ ] `src/agent/router.rs`: add focused unit coverage for streaming transport helper boundaries that do not require live router access.
   - Scope: `src/agent/router.rs` test module only.
   - Done when: tests cover `build_streaming_http_request(...)` content-length parsing instead of brittle full-string-only matching and `finalize_streaming_response(...)` success/non-200/missing-done behavior using in-memory HTTP responses.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test router::tests -- --test-threads=1`.
104. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after items 101-103 land, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
