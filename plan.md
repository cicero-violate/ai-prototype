...
104. [ ] `src/api/transport.rs`: route `ApiTransportLedger::receipt_for(...)` and `ApiTransportLedger::receipt_for_request_id(...)` through one private receipt lookup helper while preserving frame-match and request-id lookup semantics.
