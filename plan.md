...
Planning reconnaissance on 2026-05-13 confirms items 60 through 148 are complete with targeted validation, reconciliation evidence, graph-derived score refresh evidence, or graph-backed inspection evidence as recorded below. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 8.02 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next executable work should prefer small, graph-backed decomposition with validation evidence. Item 143 reconciled the already-present `../chatgpt-mcp-connector/src/tools.rs` extraction matching the graph-backed `tools::execute_evaluator_suite_tool_inner(...)` split candidate; item 145 confirmed `tools::execute_durable_policy_selected_suite(...)` via `SplitFn id=7f8fe031079d5622`, expected range `40132..45772`, rejected generated names as direct instructions, and recorded the item-146 manual helper boundary. Item 146 reconciled the already-present private helper `finalize_durable_policy_selected_evaluator_outcome(...)`; targeted policy-selected evaluator validation passed with 7 tests and 0 failures. Item 147 refreshed graph-derived score evidence after item 146; project-level numeric scores remain unchanged because this is structural evidence refresh rather than a score-history-worthy capability change. Item 148 completed the durable caller-selected evaluator helper-boundary inspection. The first incomplete Active Priorities item is item 149, the source-availability blocker check for the sibling connector source path required by the item-150 extraction. Do not apply generated names or merge-surface recommendations blindly.
...
Graph-derived score evidence from `SCORE_REPORT.md` was refreshed on 2026-05-13 after item 146: schema version 16, 16 crates, `G = 8.02 / 10`, Architecture `8.9`, Structure `4.8`, Simplicity `7.4`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.5`. The refreshed `ai` crate row reports 5,474 nodes, 33,942 edges, 2,210 functions, Architecture `9.3`, Structure `6.0`, Simplicity `7.6`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.6`. This refresh does not justify a project-level numeric score increase by itself. Item 148 completed the durable caller-selected evaluator helper-boundary inspection. Planning reconnaissance on 2026-05-13 confirmed the mounted connector workspace contains `state/rustc/auto-refactor/..__state__rustc__chatgpt_mcp_connector__bin__graph.graph-editor-plan.json`, but does not expose the sibling source path `../chatgpt-mcp-connector/src/tools.rs` or manifest `../chatgpt-mcp-connector/Cargo.toml`. The first incomplete Active Priorities item is now item 149, the source-availability blocker check; the durable caller-selected evaluator helper extraction moves to item 150 and remains gated on source availability.
...
149. [ ] `../chatgpt-mcp-connector/src/tools.rs`: resolve or document the source-availability blocker for the durable caller-selected evaluator helper extraction.
...
150. [ ] `../chatgpt-mcp-connector/src/tools.rs`: extract the private helper selected by item 148 from `execute_durable_selected_suite(...)` without changing durable caller-selected evaluator semantics.
...
151. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 150 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `plan.md`, and `status.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated from current `state/rustc` artifacts, `status.md` records aggregate and affected crate rows, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy scoring change.
   - Validation: `cargo run --manifest-path ../score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"`.
