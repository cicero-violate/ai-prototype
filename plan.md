...
Planning reconnaissance on 2026-05-12 confirms items 60 through 77 are complete with targeted and broader validation as recorded below. The next executable item is item 78, a graph-backed inspection of `src/agent/cycle.rs::AgentCycle::run(...)` using `SplitFn id=918a1611235eccfd`; the expected follow-up item is a helper extraction around the Recovery phase branch only, preserving runtime phase dispatch and evidence submission behavior. Item 76 inspected `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` and confirmed `SplitFn id=d535999f445621fb` for `agent::loop_driver::sync_mcp_workspace` with `expected_lo=22875`, `expected_hi=23972`, generated names `sync_mcp_workspace__parse` and `sync_mcp_workspace__transform`, and `delegate_strategy=preserve_original_signature`. Item 77 then extracted `parse_mcp_workspace_status(response: &str) -> u16` with passing targeted MCP workspace tests, `cargo fmt --check`, and full-suite validation. Generated `merge_surface` operations remain deferred and must be filtered manually.
...
78. [ ] `src/agent/cycle.rs`: inspect graph-backed `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` and write the smallest helper-extraction implementation checklist item for that exact function.
   ...
   - Validation: inspect `SplitFn id=918a1611235eccfd` (`fn_path=agent::cycle::AgentCycle::run`, `expected_lo=3897`, `expected_hi=12284`, generated names `run__parse` and `run__transform`, `delegate_strategy=preserve_original_signature`), inspect `src/agent/cycle.rs::AgentCycle::run(...)`, and run `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test agent::cycle::hash_tests -- --test-threads=1`.

79. [ ] `src/agent/cycle.rs`: extract the Recovery phase branch from `AgentCycle::run(...)` into private method `run_recovery_phase(&mut self, loop_steps: u64, domain: &str, state_body: &str) -> Result<Option<StopReason>, CycleError>` while preserving `AgentCycle::run() -> Result<AgentRunSummary, CycleError>` behavior.
