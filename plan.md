...
Planning reconnaissance refresh on 2026-05-14 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; listed current `../state/rustc/auto-refactor/*.graph-editor-plan.json` paths; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json`; inspected `src/capability/llm/ollama.rs` and current Ollama tests in `src/lib.rs`; and checked the working tree. There was no first incomplete executable item before this planning turn: Active Priorities items 46 through 48 were complete and `status.md` confirmed checklist exhaustion. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan. Current graph-derived aggregate evidence remains `G = 7.99 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `7.1`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.4`; Structure remains the lowest aggregate axis. Existing uncommitted non-planning changes in `GOAL.md` and `src/agent/loop_driver.rs` are outside this planning scope and must not be overwritten by planning edits.

The selected `ai` graph-editor plan remains schema version 1, graph schema version 16, and contains 1,614 planned operations. Completed or reconciled non-`root_validate` surfaces include `agent::config::{env_u32, env_u64}`, `agent::cycle::{recovery_gate, recovery_target_phase}`, `api::transport::ApiTransportLedger::{contains_request_id, has_conflicting_request}`, `capability::EvidenceSubmission::{new, with_effect}`, `capability::eval::record::{dimension_order_hash, dimension_score_hash}`, `capability::judgment::record::PolicyJudgmentRecord::{decision, judgment_record}`, `capability::judgment::record::PolicyReuseCostCatalogReceipt::{from_source_hashes, is_valid}`, `capability::learning::promote::<impl PolicyStore>::{promote, promote_feedback, promote_durable, promote_feedback_durable}`, and `graph_mutation::GraphMutationOpRow::{is_self_consistent, new}`. `agent::loop_driver::LoopDriver::run_cycle` split operation `001e821dc83e940a` remains rejected for this turn because `src/agent/loop_driver.rs` already has unrelated uncommitted edits. Adjacent `agent::cycle` route/hash wrapper candidates remain rejected because current source already has shared route/hash helpers or because structurally similar wrappers represent distinct semantic route domains. API transport receipt/session, judgment hash, policy-reuse receipt hash, receipt encoder, codec enum converter, domain bridge target, and graph mutation encoder candidates remain rejected for this turn because structurally similar wrappers represent distinct semantic domains or record families.

Graph operation `3371f13b045742b2` identifies `capability::llm::ollama::OllamaConfig::{base_url_id, model_id}` as the next safe non-root consolidation area. The safe subset is hash-delegation only: both public methods may route through a private text-id helper that wraps the existing provider text hash behavior. Do not collapse the two public methods into one semantic API: `model_id(...)` is the configured model identity, while `base_url_id(...)` is the configured endpoint identity. Preserve all public signatures, environment parsing, config validation, local-endpoint validation, client construction, request JSON behavior, retry-budget behavior, receipt/proof behavior, NDJSON encoding, and network surfaces.

49. [x] `src/capability/llm/ollama.rs`: route `OllamaConfig::model_id(...)` and `OllamaConfig::base_url_id(...)` through one private text-id helper.
   - Scope: `src/capability/llm/ollama.rs` only; allowed functions are `OllamaConfig::model_id(...)`, `OllamaConfig::base_url_id(...)`, and at most one new private helper near the existing config helpers. Do not change `OllamaConfig::from_env(...)`, `OllamaConfig::validate(...)`, `OllamaConfig::chat_completions_path(...)`, `OllamaClient`, `OllamaMessage`, receipt/proof structs, public method signatures, public struct fields, request serialization, retry-budget code, HTTP/client code, NDJSON receipt code, or tests in this item.
...
51. [ ] `SCORE_REPORT.md`: after items 49 and 50 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
