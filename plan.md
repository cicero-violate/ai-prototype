...
Planning reconnaissance on 2026-05-13 confirms items 60 through 110 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 111, focused inspection of `src/agent/cycle.rs::AgentCycle::run(...)` before extracting any helper. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next work should continue small, graph-backed decomposition with validation evidence. Current auto-refactor evidence in `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json` contains one live `SplitFn id=918a1611235eccfd` for `agent::cycle::AgentCycle::run` (`expected_lo=3897`, `expected_hi=9820`, generated names `run__parse`/`run__transform`). Treat the generated names as evidence only. Source inspection shows the safe candidate boundary is narrower: isolate deterministic phase dispatch for one observed phase while preserving objective validation, worker health gating, planning turn, observe loop limits, stop-reason semantics, final observation, summary construction, and public error behavior in `AgentCycle::run(...)`.
...
111. [ ] `src/agent/cycle.rs`: inspect live graph-backed `agent::cycle::AgentCycle::run(...)` and record the exact helper boundary for the next execution item.
   ...
   - Validation: `cargo check`.
112. [ ] `src/agent/cycle.rs`: extract a private helper for one observed phase dispatch from `AgentCycle::run(...)` without changing run-loop semantics.
   - Scope: `src/agent/cycle.rs::AgentCycle::run(...)` and one private helper in the same file. Do not change objective validation, worker health gating, `run_planning_turn(...)`, loop limit semantics, observe timing, final observation, `AgentRunSummary` construction, public error variants, or `StopReason` string behavior.
   - Done when: `AgentCycle::run(...)` delegates only phase-specific action for the already observed `phase`, `loop_steps`, `domain`, `metric`, `state_body`, and `invariant_submitted` state to the helper, while keeping stop-reason assignment and loop exit semantics equivalent.
   - Validation: `cargo check && cargo fmt --check && TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
113. [ ] `src/agent/cycle.rs`: add focused unit coverage for the extracted phase-dispatch helper using existing pure parsing/gate helpers or minimal deterministic fixtures.
