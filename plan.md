...
1. [ ] `src/agent/config.rs`: replace the duplicate `env_u32(...)` and `env_u64(...)` wrappers with direct typed uses of `env_parsed::<u32>(...)` and `env_parsed::<u64>(...)` in `AgentLoopConfig::from_env(...)`.
