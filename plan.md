...
160. [ ] `src/validation_harness.rs`: route `policy_reuse_scaling_projection_hash(...)` and `policy_reuse_distillation_readiness_hash(...)` through one private ordered receipt-field hash helper while preserving distinct projection and readiness domains.
