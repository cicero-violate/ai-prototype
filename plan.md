...
Planning reconnaissance on 2026-05-12 confirms items 60 through 70 are complete with targeted and broader validation as recorded below. The first incomplete executable item is item 71, a scoped repository-state cleanup for `src/agent/router.rs`. `git status --short` reports an unstaged router diff that exactly matches the item 69 `build_streaming_http_request(...)` extraction and focused builder test already recorded as passed. Before starting another graph-backed split, the next execution turn must either validate and commit that router source state or revert it if validation fails. This preserves clean evidence boundaries and prevents the next graph-backed refactor candidate from being layered on top of uncommitted source. Current auto-refactor reconnaissance still identifies legitimate future candidates in `state/rustc/auto-refactor/..__state__rustc__ai__graph.graph-editor-plan.json`, especially `graph_mutation::generate_graph_patch` and `capability::tooling::record::process::LiveSandboxProcessExecutor::execute_authorized_process`, but those should remain deferred until the working tree is clean.
...
71. [ ] `src/agent/router.rs`: close the uncommitted item 69 router helper extraction state before starting another graph-backed split.
