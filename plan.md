...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 116 through 118 are complete and that no unchecked executable item remains before this update; inspected current `../state/rustc/auto-refactor/*.graph-editor-plan.json`; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; inspected candidate source and tests in `src/agent/cycle.rs`; checked current graph-derived structural evidence in `SCORE_REPORT.md`; and checked the working tree. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,564 planned operations. Current planning selected graph operations `21edae3cb3c10f1b`, `96af74928d58e034`, and `447858122f87cb5a`, covering `agent::cycle::{compute_envelope_hash, compute_evidence_contract_hash, compute_submit_evidence_command_hash}`, as the next safe non-`root_validate` consolidation area. The safe subset is private submit-evidence hash wrapper input construction only: keep all three wrapper function names and signatures, preserve each distinct `HashDomain`, preserve all ordered field vectors and known hash values, preserve `compute_ordered_submit_evidence_hash(...)`, `compute_hash_domain_vector(...)`, and `compute_domain_contract_hash(...)`, preserve `build_submit_evidence_json(...)` output, and leave `root_validate` unchanged.

119. [x] `src/agent/cycle.rs`: add a private helper that centralizes submit-evidence hash wrapper delegation for `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, and `compute_envelope_hash(...)` without changing their public/private signatures.
   - Scope: `src/agent/cycle.rs` only; allowed production functions are `compute_evidence_contract_hash(...)`, `compute_submit_evidence_command_hash(...)`, `compute_envelope_hash(...)`, and one new private helper adjacent to `compute_ordered_submit_evidence_hash(...)`. Do not change `compute_structural_payload_hash(...)`, `compute_ordered_submit_evidence_hash(...)`, `compute_hash_domain_vector(...)`, `compute_domain_contract_hash(...)`, `HashDomain`, `build_submit_evidence_json(...)`, route tables, agent state transitions, tests, other modules, or `root_validate` in this item.
   - Done when: `compute_evidence_contract_hash(...)` still uses `HashDomain::EvidenceContract` and folds gate id, evidence id, passed flag, effect id, and payload hash in that order; `compute_submit_evidence_command_hash(...)` still uses `HashDomain::SubmitEvidenceCommand` and folds only the sub-contract hash; `compute_envelope_hash(...)` still uses `HashDomain::CommandEnvelope` and folds command id then command-contract hash; all three wrappers delegate through the new private helper; known hash values in existing tests remain unchanged; and no JSON, TLog, filesystem, process, network, or public API behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

120. [x] `src/agent/cycle.rs` test `submit_evidence_hash_wrapper_helper_preserves_domain_inputs`: add focused regression coverage for the new wrapper-delegation boundary.
   - Scope: `src/agent/cycle.rs` test module only. Do not change production code in this item.
   - Done when: the named test proves the evidence-contract, submit-evidence-command, and command-envelope wrappers equal explicit `compute_ordered_submit_evidence_hash(...)` calls for the Invariant/InvariantProof and Plan/TaskReady fixture paths; proves the three domains remain distinct for the same scalar input; proves reordering evidence-contract fields and command-envelope fields changes the result; proves `build_submit_evidence_json(...)` still contains the expected envelope `command_hash`; and performs no network I/O, filesystem I/O outside normal cargo test execution, environment mutation, thread spawning, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test submit_evidence_hash_wrapper_helper_preserves_domain_inputs -- --test-threads=1`.

121. [x] `SCORE_REPORT.md`: after items 119 and 120 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
