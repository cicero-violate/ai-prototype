...
Planning reconnaissance on 2026-05-12 confirms that item 60 has its source change and targeted test evidence: `src/agent/loop_driver.rs::sync_mcp_workspace` delegates endpoint parsing to `parse_mcp_workspace_endpoint(...)`, and `loop_driver::tests::mcp_workspace_endpoint_parser_preserves_host_port_and_errors` passed. Broader validation remains blocked by an unrelated rustfmt diff in `src/bin/agent.rs` around `supervisor_reload_worker_port(...)` HTTP 204 fallback handling. The first incomplete executable item is now item 61, the smallest formatter/full-suite unblocker for that exact `src/bin/agent.rs` branch. The graph-backed `SplitFn id=d535999f445621fb` for `agent::loop_driver::sync_mcp_workspace` remains useful evidence for future request-body extraction, but do not advance to that extraction, `AgentCycle::run`, router, graph mutation, or process-executor split candidates until item 61 has green formatter and full-suite validation evidence or a new blocker entry explains why it cannot be completed.
...
61. [ ] `src/bin/agent.rs`: apply rustfmt-compatible formatting to the existing `supervisor_reload_worker_port(...)` HTTP 204 fallback branch without changing behavior.
   ...
   - Validation: `cargo fmt --check`, then `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test --all-targets`.
62. [ ] `src/agent/loop_driver.rs`: extract request-body construction from `sync_mcp_workspace` into private helper `build_mcp_workspace_request(host: &str, port: u16, project_dir: &Path) -> String` after item 61 is green.
