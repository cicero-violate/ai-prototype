...
143. [ ] `src/capability/policy/store.rs`: route `PolicyProofReceipt::receipt_core_hash(...)` and `PolicyProofReceipt::verifier_context_hash(...)` through one private ordered policy proof hash-fold helper while preserving distinct proof receipt domains.
