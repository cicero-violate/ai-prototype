...
Planning reconnaissance on 2026-05-12 confirms items 60 through 91 are complete with targeted and broader validation as recorded below. The next executable item is item 93, a blocker-handling restore of `../chatgpt-mcp-connector/src/mcp_guard.rs::ai_worker_port()` to environment-only behavior before item 94 completes validation for the already-applied `../chatgpt-mcp-connector/src/tools.rs::shell(...)` helper extraction. Current `SplitFn id=ce8d33f07ffdaae6` for `tools::shell` remains legitimate graph-backed evidence, and the scoped `tools.rs` helper extraction is already present in the sibling connector working tree. Validation is blocked by an unrelated dirty `src/mcp_guard.rs` change that defaults `AI_WORKER_PORT` to `9100`, causing shell calls to fail during receipt recording with `InvalidCommand`; generated `merge_surface` operations remain deferred and must be filtered manually.
...
93. [ ] `../chatgpt-mcp-connector/src/mcp_guard.rs`: resolve the out-of-scope MCP receipt-wrapper blocker by restoring `ai_worker_port()` to environment-only behavior, removing the default `Some(9100)` path that forces local planning shell calls through supervisor receipt posting when `AI_WORKER_PORT` is unset.
