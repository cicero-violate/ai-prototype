...
Planning reconnaissance refresh on 2026-05-15 re-read `plan.md`, `status.md`, `score.md`, and `SCORE_REPORT.md`; confirmed Active Priorities items 119 through 121 are complete and that no unchecked executable item remained before this update; inspected the selected `../state/rustc/auto-refactor/workspace__ai_sandbox__canon-mini-agent__prototype__state__rustc__ai__graph.graph-editor-plan.json` with Python; inspected current `SCORE_REPORT.md`; inspected candidate source and tests in `src/capability/llm/ollama.rs`; and checked the working tree. User direction remains explicit: do not continue `root_validate` work. `root_validate` remains the weakest graph row, but it is intentionally non-selectable for this plan.

Current graph-derived aggregate evidence remains `G = 7.93 / 10`, Architecture `8.9`, Structure `4.9`, Simplicity `6.9`, Maintainability `10.0`, Determinism `10.0`, and Coherency `8.2`; Structure remains the lowest aggregate axis. The selected `ai` graph-editor plan is schema version 1, graph schema version 16, and currently contains 1,564 planned operations. Current planning skipped previously completed or stale high-ranked rows for `agent::cycle`, `agent::loop_driver`, `agent::objective`, `api::server`, `api::transport`, `capability::eval::record`, `capability::judgment::record`, `capability::learning::promote`, and already-covered Ollama hash/message/encoder helpers. Current planning selected graph operations `92041997174a63c4` and `dcaace3e381dfe27`, covering `capability::llm::ollama::{load_ollama_judgment_proof_events_ndjson, load_ollama_llm_effect_receipts_ndjson, load_ollama_llm_effect_receipts_ndjson_unchecked}`, as the next safe non-`root_validate` consolidation area. The safe subset is private Ollama NDJSON loader dispatch only: keep all public and private loader names and signatures, preserve receipt/proof record tags, preserve checked versus unchecked LLM receipt decoder behavior, preserve missing-file `Ok(Vec::new())`, preserve blank-line skipping, preserve invalid-record error mapping, preserve verification and append behavior, and leave `root_validate` unchanged.

122. [x] `src/capability/llm/ollama.rs`: route `load_ollama_llm_effect_receipts_ndjson(...)`, `load_ollama_llm_effect_receipts_ndjson_unchecked(...)`, and `load_ollama_judgment_proof_events_ndjson(...)` through one private typed Ollama NDJSON loader helper while preserving checked and unchecked decoder boundaries.
   - Scope: `src/capability/llm/ollama.rs` only; allowed production functions are `load_ollama_llm_effect_receipts_ndjson(...)`, `load_ollama_llm_effect_receipts_ndjson_unchecked(...)`, `load_ollama_judgment_proof_events_ndjson(...)`, `load_ollama_ndjson_records(...)`, and at most one new private helper or enum adjacent to those definitions. Do not change append functions, encode/decode field layouts, `parse_u64_fields(...)`, receipt/proof validity checks, verification functions, network/client code, retry-budget code, OpenAI code, tests, or `root_validate` in this item.
   - Done when: checked LLM receipt loading still filters on `OLLAMA_LLM_EFFECT_RECEIPT_RECORD` and uses `decode_ollama_llm_effect_receipt_fields(...)`; unchecked LLM receipt loading still filters on `OLLAMA_LLM_EFFECT_RECEIPT_RECORD` and uses `decode_ollama_llm_effect_receipt_fields_unchecked(...)`; proof-event loading still filters on `OLLAMA_JUDGMENT_PROOF_RECORD` and uses `decode_ollama_judgment_proof_event_fields(...)`; missing files still return an empty vector; nonmatching record tags remain ignored; and no append, encode, verification, network, retry, or public API behavior changes.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo check --lib`.

123. [x] `src/capability/llm/ollama.rs` test `ollama_ndjson_loader_helper_preserves_checked_unchecked_boundaries`: add focused regression coverage for the shared loader dispatch boundary.
   - Scope: `src/capability/llm/ollama.rs` test module only; use existing valid Ollama receipt/proof fixture helpers and temporary files under normal cargo test temp paths. Do not change production code in this item.
   - Done when: the named test writes representative valid LLM receipt and proof-event NDJSON records plus an unrelated record tag; proves checked LLM receipt loading returns only valid checked receipts; proves proof-event loading returns only valid proof events; proves unchecked LLM receipt loading returns unchecked receipts for the same LLM record tag; proves nonmatching tags are ignored; proves a missing path returns an empty vector for all three loader paths; and performs no network I/O, environment mutation, or process spawning.
   - Validation: `TMPDIR="$PWD/target/test-tmp" RUSTC_WRAPPER="" RUSTC_WORKSPACE_WRAPPER="" cargo test ollama_ndjson_loader_helper_preserves_checked_unchecked_boundaries -- --test-threads=1`.

124. [x] `SCORE_REPORT.md`: after items 122 and 123 land, refresh graph-derived structural evidence and review whether `score.md` rationale changes without raising project-level scores absent capability evidence.
