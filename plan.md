...
40. [ ] `src/capability/learning/promote.rs`: route durable policy promotion methods through the shared checked promotion-entry builder.
...
41. [ ] `src/lib.rs` test `policy_promotion_durable_entry_helpers_preserve_source_and_feedback_entries`: add durable regression coverage for the shared promotion-entry builder boundary.
