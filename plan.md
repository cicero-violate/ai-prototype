...
Planning reconnaissance on 2026-05-13 confirms items 60 through 122 are complete with targeted and broader validation as recorded below. The first incomplete Active Priorities item is item 123, a planning/inspection item for the graph-backed `examples/ollama_tool_loop_trace.rs::submit_ollama_tool_calls(...)` SplitFn candidate. Current graph-derived evidence in `SCORE_REPORT.md` reports `G = 7.93 / 10`; Structure remains the lowest aggregate axis at `4.8`, so the next work should continue small, graph-backed decomposition with validation evidence. Current auto-refactor evidence in `state/rustc/auto-refactor/..__state__rustc__ollama_tool_loop_trace__bin__graph.graph-editor-plan.json` exposes `SplitFn id=a0f85bbd8f5f9cf1` for `submit_ollama_tool_calls` with fan-out `35`, rank `70`, expected range `3474..6172`, generated names `submit_ollama_tool_calls__parse`/`submit_ollama_tool_calls__transform`, and phases `parse`/`transform`; generated names remain evidence only. Source inspection confirms the likely safe extraction boundary is one private per-tool-call helper inside `examples/ollama_tool_loop_trace.rs` that receives the already constructed `LiveMcpCallExecutor`, `OllamaClient`, `mcp_worker_url`, `mcp_receipt_path`, and `tool_call_index`, then performs the existing `tool_spec(...)`, LLM intent request/response validation, MCP call execution, MCP receipt persistence, and receipt return. `submit_ollama_tool_calls(...)` should retain MCP worker URL resolution, executor configuration, receipt vector ownership, loop control, and final `submit_ollama_mcp_evidence(...)` call. Item 123 should record the exact boundary with compile evidence; item 124 should perform the helper extraction only after item 123 passes; item 125 refreshes graph-derived score evidence after item 124 lands.
...
123. [ ] `examples/ollama_tool_loop_trace.rs`: inspect live graph-backed `submit_ollama_tool_calls(...)` and record the exact helper boundary for the next execution item.
...
124. [ ] `examples/ollama_tool_loop_trace.rs`: extract private helper `execute_one_ollama_tool_call(...)` from `submit_ollama_tool_calls(...)` for a single tool-intent/MCP-execution/result cycle only.
...
125. [ ] `SCORE_REPORT.md`: refresh graph-derived structural evidence after item 124 lands, then review whether `score.md` rationale should change without raising project-level scores absent capability evidence.
   - Scope: `SCORE_REPORT.md`, `score.md`, `status.md`, and `plan.md` only.
   - Done when: `SCORE_REPORT.md` is regenerated or confirmed current, `status.md` records the new structural evidence, and `score.md` changes only if the refreshed evidence justifies a score-history-worthy capability update.
   - Validation: `cargo run --manifest-path score/Cargo.toml --quiet -- --artifact-root state/rustc --report SCORE_REPORT.md --date "$(date +%Y-%m-%d)"` plus `git diff -- SCORE_REPORT.md score.md status.md plan.md`.
